...
function Start(startCallback) {
	...
	// This is the path to the method that calls back to the backend
	let invokerPath = config.invoker;
	let invoker = resolvePath(invokerPath);

	// Returns the function referenced by the invoker Path
	function resolvePath() {
		function resolvePathSegments(pathSegments, method) {
			let newMethod = method[pathSegments[0]];
			if (pathSegments.length === 1) return newMethod;
			return resolvePathSegments(pathSegments.slice(1), newMethod);
		}
		return resolvePathSegments(invokerPath.split('.'), window);
	}
