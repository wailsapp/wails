...
Opens a dialog prompting the user to select multiple directories. Can be customised using [OpenDialogOptions](#opendialogoptions).

Go: `OpenMultipleDirectoriesDialog(ctx context.Context, dialogOptions OpenDialogOptions) ([]string, error)`

Returns: An array of selected directories (blank if the user cancelled) or an error.
