...
If the problem still exists on the macOS operating system (`zsh: command not found`), it is probably because your file is in the `~/go/bin/wails` directory. To fix this, you can either copy the file to a directory that is already in your PATH:

```shell
cp ~/go/bin/wails /usr/local/go/bin/
```

or edit `~/.zshrc` file and add:

```shell
alias wails='/home/{user}/go/bin/wails'
```

[source](https://github.com/wailsapp/wails/issues/2046)
