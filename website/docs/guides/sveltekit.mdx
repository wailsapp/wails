...
- Change your packange manager frontend here if not using npm.
...
if [[ -z $brand ]]; then
	$brand mv frontend/src/App.svelte +page.svelte
	$brand sed -i "s|'./assets|'\$lib/assets|" +page.svelte
	$brand sed -i "s|'../wails|'\$lib/wails|" +page.svelte
	$brand mv frontend/src/assets .
fi
rm -r frontend
$manager create svelte@latest frontend
if [[ -z $brand ]]; then
	$brand mv +page.svelte frontend/src/routes/+page.svelte
	$brand mkdir frontend/src/lib
	$brand mv assets frontend/src/lib/
fi
cd frontend
$manager i
