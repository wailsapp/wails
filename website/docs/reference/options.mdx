...
By default, the desktop app will open during `wails dev`.

Setting this to `true` means the desktop will not open during `wails dev`. Please use http://localhost:34115

to debug you app.

NOTE: Note: If this option is set to true in development mode, it will always be hidden and will not be affected by the `StartHidden` option.