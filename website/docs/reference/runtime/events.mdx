...
```go title=Backend
    runtime.EventsOn(ctx, "save", func(data ...interface{}) {
        dataToSave := data[0].(string)
        // Save file
    })
```
```js title=Frontend
    var myData = "Hello!"
    myElement.click(() => {
        runtime.EventsEmit("save", myData)
    })
```
...
```go title=Backend
    runtime.EventsEmit(ctx, "load-started")
    // Load big file
    runtime.EventsEmit(ctx, "load-finished")
```
```js title=Frontend
    runtime.EventsOn("load-started", showSpinner);
    runtime.EventsOn("load-finished", hideSpinner);
```