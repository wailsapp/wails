...
On windows when the title and message strings contain special characters (eg. á, í, ű, ő) they need to be encoded to a
windows character map for example windows-1250 otherwise the notification can show the wrong characters.
...
Type: `*runtime.WindowsNotificationOptions`
...
The current implementation only shows the actions, signals are not handled currently.
...
Name: Actions<br/>
Type: `[]runtime.MacNotificationAction`
...
Available options:
 ...
 - LinuxNotificationUrgencyLow
 - LinuxNotificationUrgencyNormal
 - LinuxNotificationUrgencyCritical

Name: Urgency<br/>
Type: [`runtime.LinuxNotificationUrgency`](#linuxnotificationurgency-type)

##### LinuxNotificationUrgency type
...
| Value                            | Description |
| ---------------------------------| ----------- |
| LinuxNotificationUrgencyLow      | For low urgencies, server implementations may display the notifications how they choose. |
| LinuxNotificationUrgencyNormal   | For normal urgencies, server implementations may display the notifications how they choose. |
| LinuxNotificationUrgencyCritical | Critical notifications are not expired automatically, they close when the user dismisses them, for example, by clicking on the notification. |
...
Name: Actions<br/>
Type: `[]runtime.LinuxNotificationAction`

##### Action Key

Key is the actions' identifier used by the notification server.
...
Name: Actions<br/>
Type: `*runtime.LinuxNotificationSound`
