...
To create a new Wails application using the default vanilla JS template, you
...
To build the application, change to the new `helloworld` project directory and
...
If we view the `build/bin` directory in Windows Explorer, we should see our
project binary:
...
On Mac, Wails generates a `helloworld.app` file which can be run by
double-clicking it.

On Linux, you can run the application using `./helloworld` from the `build/bin`
directory.
