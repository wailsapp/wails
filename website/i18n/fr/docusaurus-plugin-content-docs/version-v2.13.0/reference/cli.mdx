...
| -ide                      | Générer les fichiers du projet IDE `vscode` ou `goland`                                                                |               |
...
:::info Définir une version minimale pour MacOS

Vous pouvez remplacer la [version minimale](../gettingstarted/installation#supported-platforms) par défaut de macOS pour votre application en fournissant une version via les variables d'environnement `CGO_CFLAGS` et `CGO_LDFLAGS`. par exemple `CGO_CFLAGS=-mmacosx-version-min=10.15.0 CGO_LDFLAGS=-mmacosx-version-min=10.15.0 wails build`
