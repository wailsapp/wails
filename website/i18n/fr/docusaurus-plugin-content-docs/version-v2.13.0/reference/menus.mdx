...
```go
    ...
    if runtime.GOOS == "darwin" {
        AppMenu.Append(menu.AppMenu()) // Sur macOS, doit être fait juste après `NewMenu()`
    }
    ...
    FileMenu.AddText("Quit", keys.CmdOrCtrl("q"), func(_ *menu.CallbackData) {
        // `rt`est un alias de "github.com/wailsapp/wails/v2/pkg/runtime" pour éviter des collisions avec le packet standard
        rt.Quit(app.ctx)
    })

    if runtime.GOOS == "darwin" {
    AppMenu.Append(menu.EditMenu())  // Sur macOS, EditMenu doit être ajouté pour activer les raccourcis Cmd+C, Cmd+V, Cmd+Z...
    }
    ...
    err := wails.Run(&options.App{
        ...
        Width:             800,
        Height:            600,
        Menu:              AppMenu, // Référence du menu au dessus
        Bind: []interface{}{
            app,
        },
    )
