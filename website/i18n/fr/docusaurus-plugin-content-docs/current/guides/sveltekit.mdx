...
1. Étapes d'installation minimales - Les étapes nécessaires pour obtenir une configuration minimale de Wails fonctionnant pour SvelteKit.
...
##### Lorsque vous êtes dans le dossier racine du projet Wails. Utilisez le CLI Svelte pour créer un projet SvelteKit comme nouveau frontend. Suivez les instructions, rien de spécifique à Wails n'est nécessaire ici.
...
- La ligne `frontend/dist` doit être remplacée par `frontend/build`
