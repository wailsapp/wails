...
Vous pouvez également utiliser des énums dans vos structures et avoir des modèles pour elles dans le frontend. Dans ce cas, vous devriez créer un tableau qui contiendra toutes les valeurs possibles de l'énumération, définir le type de l'énumération et lier le tout à l'application:
...
Dans la configuration principale de l'application, le paramètre `EnumBind` est l'endroit où nous pouvons dire à Wails ce que nous voulons lier :
...
Ceci ajoutera des enums manquants à votre fichier `model.ts`.
