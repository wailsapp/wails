...
func main() {
    ...
    err := wails.Run(&options.App{
        ...
        Windows: &windows.Options{
            ...
            // ZoomFactor est le facteur de zoom pour WebView2. Il s'agit de l'option correspondant au zoom avant ou arrière défini par l'utilisateur.
            ZoomFactor:           float64,
            // IsZoomControlEnabled permet à l'utilisateur de changer le facteur de zoom.
            IsZoomControlEnabled: bool,
            // Messages utilisateur qui peuvent être personnalisés
            Messages: *fenêtres. essages
            // OnSuspend est appelé lorsque Windows entre en mode basse puissance
            OnSuspend: func()
            // OnResume est appelé lorsque Windows reprend le mode basse puissance
            OnResume: func(),
            // Désactive l'accélération matérielle GPU pour le webview
                  WebviewGpuDisabled: false,
                  // Nom de la classe pour la fenêtre. Si vide, 'wailsWindow' sera utilisé.
...
Cela ne fonctionne pas avec la v5.0.0+ vite et wails v2 en raison des changements dans vite. Des changements sont prévus dans la v3 pour prendre en charge des fonctionnalités similaires sous la v5.0.0+. Si vous avez besoin de cette fonctionnalité, restez avec la v4.0.0+. Regardez l'[issue 3240](https://github.com/wailsapp/wails/issues/3240) pour les détails
...
Un tableau d'Enum qui doivent être liés au frontend.
...
Active le verrouillage par instance unique. Cela signifie qu'une seule instance de votre application peut être exécutée à la fois.

Nom: SingleInstanceLock<br/> Type : `*options.SingleInstanceLock`
...
Nom: UniqueId<br/> Type: `string`

#### OnSecondInstanceLaunch

Callback appelé quand une seconde instance de votre application est lancée.

Nom: OnSecondInstanceLaunch<br/> Type: `func(secondInstanceData SecondInstanceData)`

### Drag and Drop

Définit le comportement des événements de glisser-déposer sur la fenêtre.

Nom: DragAndDrop<br/> Type: `options.DragAndDrop`

#### EnableFileDrop

EnableFileDrop active la fonctionnalité de glisser-déposer des wails qui retourne les chemins absolus des fichiers déposés.

Quand il est réglé sur `true` les [méthodes d'exécution](../reference/runtime/draganddrop.mdx) peuvent être utilisées. <br/> Ou vous pouvez écouter l'événement `wails:file-drop` avec la méthode [runtime EventsOn](../reference/runtime/events.mdx#eventson) dans Javascript et GO pour implémenter toutes les fonctionnalités que vous souhaitez.

L'événement renvoie les coordonnées de la liste déroulante et le chemin d'accès du fichier.

Nom : EnableFileDrop<br/> Type : `bool`<br/> Défaut : `false`

#### DisableWebViewDrop

Désactive la fonctionnalité de glisser-déposer de la vue Web.

Il peut être utilisé pour éviter l'ouverture accidentelle des fichiers glissés dans la vue Web, quand il n'y a pas besoin de glisser-déposer.

Nom: DisableWebViewDrop<br/> Type: `bool`<br/> Défaut: `false`

#### CSSDropProperty

Propriété CSS pour tester les éléments cibles par glisser-déposer.

Nom: CSSDropProperty<br/> Type: `string`<br/> Défaut: `--wails-drop-target`

#### CSSDropValue

La valeur CSS que la CSSDropProperty doit être une cible de drop valide. Défaut "drop"

Nom: CSSDropValue<br/> Type: `string`<br/> Défaut: `drop`
...
Définir ceci à `true` désactivera les gestes de zoom pinchés.

Nom: DisablePinchZoom<br/> Type: `bool`
...
Nom : WindowClassName<br/> Type : `string`
...
Callback qui est appelé quand un fichier est ouvert avec l'application.

Nom: OnFileOpen<br/> Type: `func(filePath string)`

#### OnUrlOpen

Callback qui est appelé quand une URL est ouverte avec l'application.
