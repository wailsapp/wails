...
Une liste complète des options de l'application peut être trouvée dans la [Référence des Options](reference/options.mdx).
...
Vous pouvez également lier des types d'énumérations. Dans ce cas, vous devriez créer un tableau qui contiendra toutes les valeurs possibles de l'énumération, définir le type de l'énumération et le lier à l'application via `EnumBind`:
...
Plus de détails sur l'exécutable JS peuvent être trouvés dans la [Référence d'exécution](reference/runtime/intro.mdx).
