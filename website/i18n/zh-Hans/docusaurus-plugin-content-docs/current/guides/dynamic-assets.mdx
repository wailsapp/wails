...
```
这将在开发工具中产生一个错误。 This will generate an error in the devtools. We can see that the error is what we expect, returned by our custom assets handler:
...
这种技术可以直接将图像加载到页面。 This technique can be used to load images directly into the page. If we updated our default vanilla template and replaced the logo image:
```html
