...
it''s likely that your application''s `info.plist` is invalid. it''s likely that your application''s `info.plist` is invalid. Update the file in `build/<yourapp>.app/Contents/info.plist` and check if the data is valid, EG check the binary name is correct. To persist the changes, copy the file back to the `build/darwin` directory. To persist the changes, copy the file back to the `build/darwin` directory.
