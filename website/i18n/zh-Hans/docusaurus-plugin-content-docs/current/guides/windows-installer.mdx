...
创建新项目时，Wails 从`installer/info.json`中读取配置数据并使用项目的`wails.json`的 Info 部分，在`build/windows/installer`中生成 NSIS 配置文件： 配置 数据从 `installer/info.json` 读取，这个配置被配置为使用项目的 `wails.json` 信息部分：
