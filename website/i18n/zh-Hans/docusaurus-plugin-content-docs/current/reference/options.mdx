...
CustomTheme 结构使用 `int32` 指定颜色值. 这些是非标准 Windows 格式：
`0x00BBGGAA`。 提供了一个辅助函数来将 RGB 转换为这种格式：`windows.RGB(r,g,b uint8)`。
...
图标应该以自然绘制的任何尺寸提供；也就是说，在传递图像之前不要缩放图像。
缩放将延迟到当所需的最终尺寸已知的最后一刻，以获得最佳质量。
