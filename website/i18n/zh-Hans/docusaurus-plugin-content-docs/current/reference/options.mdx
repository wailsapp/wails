...
默认情况下，开发模式下(`wails dev`)将开启桌面端。 将此设置为 `true` 意味着开发模式下不会开启桌面端，请通过http://localhost:34115进行调试。

名称：StartHiddenOnDev<br/> 类型：`bool`

注意: 在开发模式下该选项设为true，则始终隐藏，不受`StartHidden`的影响