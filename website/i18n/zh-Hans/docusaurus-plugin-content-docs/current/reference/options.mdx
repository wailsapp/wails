...
如果设置，当 Windows 启动切换到低功耗模式（挂起/休眠）时，将调用此函数
...
名称：TitleBar<br/> 类型：[`*mac.TitleBar`](#标题栏结构)
...
名称：Appearance<br/> 类型：[`mac.AppearanceType`](#appearance-type)
...
名称：About<br/> 类型：[`*mac.AboutInfo`](#about-struct)
