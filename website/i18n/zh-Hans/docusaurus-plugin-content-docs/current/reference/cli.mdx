...
| -s              | 跳过前端构建                                                                                                                                                                         | 否                           |
| -f              | 强制构建应用                                                                                                                                                                         | 否                           |
| -tags "额外标签"    | 构建标签传递到编译器。 必须引用。 Space or comma (but not both) separated                                                                                                                      |                             |
...
| -trimpath       | 从生成的可执行文件中删除所有文件系统路径。                                                                                                                                                          | 否                           |
| -race           | Build with Go's race detector                                                                                                                                                  | 否                           |
| -windowsconsole | 保留Windows构建控制台窗口                                                                                                                                                               | 否                           |
...
| -devserver "host:port"       | 使用第三方开发服务器提供资源，比如 Vite                                                                                                           | "localhost:34115"        |
