...
主应用程序由对`wails.Run()`的调用组成. 它接受描述应用程序窗口大小、窗口标题、要使用的资源等应用程序配置。 它接受了描述应用程序窗口大小、窗口标题、 要使用的资源等的 应用程序配置。 基本应用程序可能如下所示：
...
导入 (
    ...
    "github. om/wailsapp/wails/v2"
    "github. om/wailsapp/wails/v2/pkg/options”
...
类型App struct construct {
    ctx context.Context
}

func (B*App) startup(ctx contexer)。 ontext (http://europa-eu-un.org )
    b.ctx = ctx
}

func (b*App) shutdown(ctx context ontext) {}

func (b*App) Greet(name string) string volunt.
...
当使用`wails dev`命令在“开发”模式下，资源从磁盘加载，任何更改都会导致“实时重新加载”。 资源的位置需要使用`-assetdir`传递给`wails dev`命令，并且很可能与嵌入路径相同。 希望将来我们可以从`embed.FS`本身计算出来。
...
`Bind`选项是 Wails 应用程序中最重要的参数选项之一。 它指定向前端暴露哪些结构方法。 想到传统的 web 应用程序中的结构像"控制器"。 当应用程序启动时，它会检查 `Bind` 中列出的结构实例， 确定哪些方法是公开的（以大写字母开头），并将生成前端可以调用的那些方法的 Javascript 版本。
...
当你运行 `wails dev` (或 `wills 生成模块`)时，将生成一个前端模块，其中包含以下内容：
  - 调用绑定的 Go 方法
  - 调用运行时方法
  - 按绑定方法作为输入或输出使用的所有跳转结构的类型定义

这使得从前端调用转码变得非常简单，使用相同的强化类型的数据结构。
...
在`开发`模式下运行应用程序时，会生成一个 javascript 模块，该模块用 JSDoc 注释包装这些方法。 这确实有助于开发，尤其是因为大多数 IDE 将处理 JSDoc 以提供代码完成和类型提示。 该模块名为`go` 并在`wailsjsdir`标志指定的目录中生成。 生成的文件反映了您应用程序中的 软件包名称。 在上面的例子中，我们绑定 `app`，它有一个公开方法 `Greet`。 这将 导致生成以下文件：
...
```
在这里我们可以看到有一个 `个主` 包包含了绑定的 `App` 结构的 Javascript 绑定， 以及 作为这些方法的典型声明文件。 要在我们的前端调用 `Greet` ，我们只需要导入方法和 调用它就像一个普通的 Javascript 函数：
...
这些方法返回一个 Promise。 成功的调用将导致 Go 调用的第一个返回值被传递给 `resolve` 处理程序。 一个不成功的调用是将一个 Go 方法的第二个错误类型返回值通过`reject`传递回调用者。 这将通过 `拒绝` 处理器传回。 在上面的例子中，Greet 只返回一个字符串，所以 `Javascript` 调用永远不会`reject` - 除非将无效数据传递给它。
...
type Address struct 卷.
    Street string `json:"street"
    Poststring `json:"postcode"
}

func (a.*App) Greet(p Person)字符串.
    return fmt. 打印("Hello %s (: %d)!", p.Name, p.Age)
}
```
...
我们可以看到，"主要"命名空间是从一个新的“models.ts”文件导入。 此文件包含我们绑定方法所使用的所有结构定义 。 在这个例子中，这是一个 `个人` 结构。 如果我们看到 `模型。ts`, 我们可以看到模型 是如何定义的：
...
生成的绑定和 TypeScript 模型的组合为强大的发展环境提供了条件。
...
关于JS 运行时间的更多详细信息可在 [运行时间参考](reference/runtime/intro) 中找到。
