...
- 为 `wails.json` 配置文件添加新属性 - `bindings`。 可以在更新的 [schema](static/schemas/config.v2.json) 中找到有关新属性的更多信息。 属性 `前缀` 和 `后缀` 允许您控制在 `model.ts` 文件中生成的 TypeScript 实体名称。 由 @OlegGulevskyy 在这个 [PR](https://github.com/wailsapp/wails/pull/2101) 中修复。
