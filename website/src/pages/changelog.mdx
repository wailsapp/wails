...
- Fixed a macOS crash when dragging an element inside the webview while `EnableFileDrop` is set: the drop handler read non-file URLs off the pasteboard and called `-fileSystemRepresentation` on them, which returns `NULL` and crashed the process on the following `-initWithCString:` by @JaesonMart

## v2.16.0 - 2026-09-14

### Fixed

- Fixed a memory leak in the macOS and Linux frontends where `Calloc` never freed the C strings it allocated for dialogs, menus and window setup by @4RH1T3CT0R7
...
- Added EventsOffAll to `windows.runtime`, fixes [#3136](https://github.com/wailsapp/wails/issues/3136) [PR](https://github.com/wailsapp/wails/pull/4254) by [@FrancescoLuzzi](https://github.com/FrancescoLuzzi)
- Fixed issues with hot reloading and quitting not working on Windows [PR](https://github.com/wailsapp/wails/pull/4266) by [@hkere](https://github.com/hkhere)
