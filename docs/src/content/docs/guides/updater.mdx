...
(function () {
  ...
  (function announce() {
    if (window._wails && typeof window._wails.invoke === "function") {
      ...
      emit("wails:updater:window:ready"); // tell the host to replay current state
    } else { setTimeout(announce, 30); }
  })();
})();
// Now use updater.on(name, fn) / updater.emit(name)
updater.on("wails:updater:update-available", (e) => {
    const rel = e.data ?? e;
    document.getElementById("ver").textContent = rel.version;
});
document.getElementById("install").addEventListener("click",
    () => updater.emit("wails:updater:user:install"));
</script>
```
...
Use the `Updater.Events` constants exported from `@wailsio/runtime` instead of typing the strings yourself — the names are checked at compile time (with TypeScript) and a regression test keeps the JS exports in sync with the Go-side constants.
