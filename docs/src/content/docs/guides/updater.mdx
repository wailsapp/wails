...
Drive the update flow against an `*application.WebviewWindow` you create yourself:
...
The Updater will call `Show()` / `Close()` / `EmitEvent()` on your window through the same event channel as the builtin. Your HTML is responsible for responding — same shim pattern as above. The framework doesn't touch the window's HTML if you supply your own.
