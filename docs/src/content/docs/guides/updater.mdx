...
Configured headers are sent on every manifest request; artifact downloads reuse them only on the manifest's own host, and the `Authorization` header is stripped on any cross-origin redirect.
