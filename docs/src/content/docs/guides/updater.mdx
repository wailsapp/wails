...
<Image src={defaultWindowUpToDate} alt="The default updater window in the Up-to-Date state — green checkmark, 'You're Up to Date' heading, single Close button." style="max-width: 420px; margin: 1rem auto; display: block; border-radius: 12px; box-shadow: 0 4px 20px rgba(0,0,0,0.14);" />
...
Provide your own HTML; it just needs to listen to `updater:*` events and emit `updater:user:*` actions:
...
Inside an InitialHTML window the page can't load `/runtime.js` (the base URL is not the asset server), so the standard `@wailsio/runtime` imports won't work. Use this minimal shim instead — it's what the default template uses:
...
(function () {
  ...
  w.dispatchWailsEvent = function (ev) {
    var cbs = L[ev && ev.name]; if (!cbs) return;
    for (var i = 0; i < cbs.length; i++) try { cbs[i](ev); } catch (_) {}
  };
  function on(n, cb)   { (L[n] = L[n] || []).push(cb); }
  function emit(name)  { if (typeof w.invoke === "function") w.invoke("wails:event:emit:" + name); }
  window.updater = { on: on, emit: emit };
  // Wait for the runtime to be injected, then unlock pending event delivery
  (function announce() {
    if (window._wails && typeof window._wails.invoke === "function") {
      window._wails.invoke("wails:runtime:ready");
      emit("updater:window:ready"); // tell the host to replay current state
    } else { setTimeout(announce, 30); }
  })();
})();
// Now use updater.on(name, fn) / updater.emit(name)
updater.on("updater:update-available", (e) => {
    const rel = e.data ?? e;
    document.getElementById("ver").textContent = rel.version;
});
document.getElementById("install").addEventListener("click",
    () => updater.emit("updater:user:install"));
</script>
```
...
<Image src={byoCustomWindow} alt="A 'Bring your own' updater window with a pink-orange gradient background, a single white rounded card, custom typography, and the same updater events driving the visible state. Demonstrates how completely the default UI can be replaced." style="max-width: 380px; margin: 1rem auto; display: block; border-radius: 12px; box-shadow: 0 4px 20px rgba(0,0,0,0.14);" />
...
Your HTML wires up the same shim pattern shown in [Replace the template](#replace-the-template) — the runtime injection works identically whether the window is framework-owned or yours. The framework never touches the HTML you supplied.
...
`*application.CustomEvent` has a `Data any` field. The updater sets it directly to the typed payload listed in the [event reference](#event-reference) — type-assert, don't JSON-decode:
...
Events.On("updater:download-progress", (e) => {
    const p = e.data;
...
| Event constant | Wire name | Payload | When |
|---|---|---|---|
| `EventCheckStarted` | `updater:check-started` | none | Before each `Check` round-trip |
| `EventUpdateAvailable` | `updater:update-available` | `*Release` | `Check` found a newer release |
| `EventNoUpdate` | `updater:no-update` | none | `Check` confirmed up-to-date |
| `EventDownloadStarted` | `updater:download-started` | `*Release` | Bytes start streaming |
| `EventDownloadProgress` | `updater:download-progress` | `Progress` | ~10 Hz during download |
| `EventDownloadComplete` | `updater:download-complete` | `*Release` | All bytes written, before verify |
| `EventVerifying` | `updater:verifying` | `*Release` | Signature/digest check begins |
| `EventInstalling` | `updater:installing` | `*Release` | Unpack + staging begins |
| `EventUpdateReady` | `updater:update-ready` | `*Release` | Restart pending |
| `EventError` | `updater:error` | `ErrorInfo` | Any stage failed |

User-action events emitted *by* the window template (your code subscribes if you write a custom template):

| Event | When |
|---|---|
| `updater:window:ready` | Window finished loading; host replays current state in response |
| `updater:user:install` | Primary action in `available` state |
| `updater:user:restart` | Primary action in `ready` state |
| `updater:user:skip` | "Skip This Version" |
| `updater:user:remind` | "Remind Me Later" |
| `updater:user:cancel` | Close button |
