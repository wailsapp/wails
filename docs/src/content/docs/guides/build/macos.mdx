...
There is no `wails3 task darwin:create:dmg` task in the shipped Wails 3 templates. To produce a DMG, drive a tool like `create-dmg` or `hdiutil` from your own Taskfile after running `wails3 task darwin:package` (or `darwin:package:universal`) — the `.app` bundle ends up under `bin/`.
