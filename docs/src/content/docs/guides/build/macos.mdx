...
Resource names are slash-separated paths relative to `Contents/Resources`. `ResourceFS` and `LoadResource` return `mac.ErrNotInAppBundle` when the executable is not running inside a `.app` bundle.
