...
```bash
# Build with experimental GTK4 support on Linux
...
Tags are forwarded as `EXTRA_TAGS` to the underlying Taskfile. See [Server Build](/guides/server-build) and [Linux Packaging - GTK4](/guides/build/linux#gtk4-support-experimental) for details.
