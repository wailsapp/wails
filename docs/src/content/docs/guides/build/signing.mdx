...
This writes a shared signing configuration to `~/.config/wails/defaults.yaml` that is
**honored at sign time on every platform** (see [Configuration precedence](#configuration-precedence)).
Its signing step:
- Detects which signing tools are installed for the platform you want to target —
  **from any host** — and shows the install command for your OS (e.g.
  `brew install gnupg`, `sudo apt install osslsigncode`, `winget install GnuPG.Gpg4win`).
...
When you run a sign task, each signing option is resolved in this order (first
match wins):
...
This means the Taskfile vars are **optional overrides**: if a var is unset, the
globally configured key/certificate/identity is used. If none of the three provides
a value, the sign command reports a clear error telling you how to configure it.

## Per-project configuration

To configure signing for a single project rather than globally, run the per-project
wizard from inside the project — it writes the `vars` into that project's
`build/<platform>/Taskfile.yml` files:
...
If you just need to test the signing pipeline, run `wails3 setup`, open the
**Windows** tab of the signing step, and choose **Generate a self-signed
certificate**. This uses OpenSSL to create a code-signing `.pfx` and records its
path in the global config.

<Aside type="caution">
Self-signed certificates are for **testing and internal distribution only** — they
trigger SmartScreen warnings for end users. Public releases need a certificate from
a trusted CA.
</Aside>
...
<Aside type="note">
These vars are optional **overrides**. If unset, the certificate configured globally
via `wails3 setup` is used. See [Configuration precedence](#configuration-precedence).
</Aside>
...
The easiest way is the setup wizard — run `wails3 setup`, open the **Linux** tab of
the signing step, and choose **Create a new GPG key**. The wizard:
...
- records both the key ID and the exported path in `~/.config/wails/defaults.yaml`,
  so it's used automatically at sign time.

<Aside type="note">
If a key was configured by **ID only** (e.g. created before the wizard exported keys
automatically), the Linux tab exports it to a file on your next visit and fills in
the path — no manual steps needed. Builds sign with a key *file*, so the path is
what matters.
</Aside>
