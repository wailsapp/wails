...
<Aside type="tip">
All platforms can be signed from **any platform**! macOS cross-platform signing uses the [Quill](https://github.com/anchore/quill) library and requires exporting your Developer ID certificate as a P12 file.
</Aside>
...
| macOS | `codesign` (Xcode) | Quill library |
| Linux | N/A | Built-in |
...
<Aside type="note">
**macOS signing recommendation**: On macOS, prefer native `codesign` (via `sign:` task) as it's Apple's official tooling. Use cross-platform signing (via `sign:cross:` task) only when building on Linux/Windows or in CI/CD without macOS runners.
</Aside>
...
You can sign macOS binaries from Linux or Windows using the Quill library. This is useful for CI/CD pipelines that don't have access to macOS runners.
...
Run the setup wizard (select "Cross-platform (Quill)" when prompted):
...
wails3 task darwin:sign:cross

# Sign and notarize (from any platform)
wails3 task darwin:sign:cross:notarize
```
    ...
    steps:
      ...
      - name: Store Credentials
        env:
          ...
          NOTARY_ISSUER: ${{ secrets.APPLE_TEAM_ID }}
        run: |
          # Store credentials in keychain for wails3 to use
          wails3 keychain set macos-p12-password "$P12_PASSWORD"
          wails3 keychain set notary-key-id "$NOTARY_KEY_ID"
          wails3 keychain set notary-issuer "$NOTARY_ISSUER"
      ...
      - name: Sign and Notarize
        run: |
          wails3 task darwin:sign:cross:notarize \
            P12_CERTIFICATE=developer-id.p12 \
            NOTARY_KEY=AuthKey.p8
...
<Aside type="note">
Cross-platform signing produces identical results to native signing. The signed binary will pass Apple's notarization and Gatekeeper checks.
</Aside>
