...
`wails3 build` itself has no `-ldflags` flag — the shipped Taskfile already passes `-ldflags="-s -w"` to `go build`. To further shrink the binary, edit the build task or call `go build` directly:
