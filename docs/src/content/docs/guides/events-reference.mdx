...
Handle theme changes in Go and forward to the frontend:
...
You can also listen for the system event directly in JavaScript, but note that the theme data is not included:

```javascript
Events.On('common:ThemeChanged', () => {
    // Theme changed, but isDark is not available
    // Use CSS media queries or query the System API instead
});
```
