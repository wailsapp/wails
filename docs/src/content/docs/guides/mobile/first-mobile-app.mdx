...
- **Go 1.24+** and **npm** (already installed if you ran `wails3 init`)
...
1. **Launch the app**
   ...
   On first run this:
   - Generates the Xcode project scaffold in `build/ios/`
   - Compiles your Go code as a C archive (`GOOS=ios GOARCH=arm64`)
   - Bundles the frontend
   - Signs ad-hoc
   - Boots a simulator (if none is running)
   - Installs and launches your app

   Subsequent runs are much faster — only changed files recompile.

   <Aside type="tip">
   The first build takes 2–5 minutes. Go is compiling the entire Wails framework for `GOOS=ios` for the first time and caching it. Future builds of any iOS app will be significantly faster.
   </Aside>
