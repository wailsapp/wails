...
It demonstrates every major mobile API surface across 7 tabs:

| Tab | What it shows |
|---|---|
| **Bindings** | JS → Go service calls returning values, structs, and errors |
| **Events** | Go → JS clock, JS → Go → JS ping/pong, OS system events (battery, network, theme) |
| **Dialogs** | Native message dialogs on each platform |
| **System** | Clipboard, screen metrics, device info |
| **Mobile** | Share sheet, keep-awake, torch, brightness, biometrics, local notifications, secure storage |
| **Hardware** | Haptics, geolocation, accelerometer, proximity, text-to-speech |
| **Native** | iOS: haptics + WKWebView toggles · Android: vibrate + toast |
