...
It demonstrates every major mobile API surface across 7 tabs — and it runs on desktop too. The **Mobile** and **Hardware** tabs are hidden on desktop via a platform check in the frontend; the Go side registers no handlers for `native:*` events when built for desktop. This is the recommended pattern for shipping one codebase everywhere.
