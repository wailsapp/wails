...
**Buffer ownership.** `send()` does not copy. A frame is queued behind any sends already
in flight, and the bytes are read when the request is issued, so do not mutate or reuse a
buffer you have passed to `send()` until it has gone out. Copying every frame was measured
to cost more than it was worth: at several hundred MB/s the allocation churn dominates the
transport. Pass a fresh buffer, or a string, if you want to reuse storage.
