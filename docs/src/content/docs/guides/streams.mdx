...
**Buffer ownership — neither side copies.** `Send` queues your slice and `send()` queues
your buffer; the bytes are read when the frame is written. Do not mutate or reuse either
until the frame has gone out. A real WebSocket copies on send, and this deliberately does
not: matching the API shape is the goal, and a memcpy per frame is the single largest cost
at these rates — removing it took JS→Go from 1,568 to 2,793 MB/s. Hand over a fresh buffer,
or a string, if you want to reuse storage.
...
transport for you before any module code runs.
...
- **Go→JS is much faster for small frames** — 634,000 frames/s on macOS against ~6,200/s
  the other way. A single response coalesces up to 256 frames; every JS→Go frame is its
  own request. If you are sending many small messages, prefer Go→JS, or batch them
  yourself before sending up.
...
| | limit |
|---|---|
| Buffered per window, awaiting collection | 8 MB or 256 frames, whichever first |
| Received per connection, awaiting `Receive` | 8 MB or 256 frames — the frontend is then made to wait |
| A single frame, Go→JS | no limit — an empty buffer accepts any size |
| A single frame, JS→Go | 64 MB |
| Idle poll hold | 20 s |
