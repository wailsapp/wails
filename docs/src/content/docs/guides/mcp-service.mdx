...
For project lifecycle automation, use the separate `wails3 mcp` CLI server. It lets
agents inspect and initialize projects, run diagnostics, start builds and development
jobs, generate bindings, run named Taskfile tasks, and retrieve bounded job output.
The CLI server is confined to the current directory by default and does not expose
arbitrary shell execution. See the [CLI MCP documentation](/guides/cli/#mcp) for transport,
authentication, and tool details.
