...
Or wrap once and leave every handler untouched — see [the compatibility shim](#compatibility-shim).
...
export function TextStream(name) {
    ...
    s.addEventListener("message", (ev) => {
        ...
        const text = dec.decode(ev.data);
        // Re-dispatch as a string message for handlers assigned to onmessage.
        if (typeof s.onmessage === "function") {
            s.onmessage(new MessageEvent("message", { data: text }));
        }
    });

    // Prevent the raw ArrayBuffer event from also reaching onmessage.
...
## If your WebSocket talks to an external service (NATS, MQTT, a broker)

If the frontend currently opens a WebSocket **to a broker rather than to your app** — for
example `nats.ws` connecting to a NATS server — a stream is not a drop-in, because a
stream connects the frontend to *your Go code*, not to a third party.
