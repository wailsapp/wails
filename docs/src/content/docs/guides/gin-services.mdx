...
<body>
    ...
    <script type="module">
        ...
        window.addEventListener('DOMContentLoaded', () => {
            ...
            wails.Events.On("gin-api-response", (event) => {
                document.getElementById('eventResponse').textContent = JSON.stringify(event.data, null, 2);
            });

            // Also listen for user-created events
            wails.Events.On("user-created", (event) => {
                document.getElementById('eventResponse').textContent = JSON.stringify({
                    event: "user-created",
                    user: event.data
                }, null, 2);
