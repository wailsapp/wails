...
### `init`
Initializes a new Wails project. During this initialization, the `go mod tidy` command, which is typically used to add missing and remove unused modules, is intentionally bypassed. This is done automatically as part of the initialization process.
...
| `-skipgomodtidy`      | Skip go mod tidy     | `true`                   |
