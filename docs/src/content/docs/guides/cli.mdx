...
Starts the Wails project MCP server for agent-assisted project management. This is
...
- When run interactively in a terminal, the server uses **Streamable HTTP** on
  `127.0.0.1` and asks the operating system for a free port.
...
| `-token` | Session/bearer token for mutating and process-control tools. | Generated securely |
