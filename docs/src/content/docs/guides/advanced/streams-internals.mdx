...
Receive() ◄─ per-conn inbox ◄──────── POST /wails/stream/send   (one per frame)
```
...
**JS→Go is an ordinary POST per frame.** Sends are serialised per connection with a promise
chain, because concurrent `fetch` calls do not preserve order and Go relies on send order
being the order it observes. Go appends to the connection's inbox *before* responding, so
the client cannot issue the next send until the previous one is queued.
...
| `streamSessionTTL` | 60 s | no poll for this long ⇒ session is dead |
| `streamMaxSendBytes` | 64 MB | one JS→Go frame |
...
**The inbound bound is what makes the frontend wait.** `deliver` blocks while a
connection's inbox is full, which on the desktop leaves the client's `fetch` unresolved so
it cannot post again, and in server mode stalls the socket read pump so TCP does the rest.
Without it a handler slow to call `Receive` let the frontend grow host memory without
limit.

**Control frames bypass every cap.** Losing a data frame under backpressure is a
slow-down; losing an open ack leaves the frontend in `CONNECTING` forever, and losing a
close leaves it believing a dead connection is live.

**Frames are copied on the way in.** `Send` reports acceptance before a poll encodes the
frame, so retaining the caller's slice would let a pooled or reused buffer alter a frame
already acknowledged. The client snapshots at its `send()` boundary for the same reason.
...
1. **A new session for a window supersedes the old one.** A reload gives the page a new
   session id; the previous session's connections close immediately. Measured over six
   reloads: 7 connects, 6 disconnects, never more than one live at a time, and the old
   connection closes *before* its replacement appears.
...
| JS→Go frame coalescing | not done — every frame is its own POST, which is why small-frame upload rate is ~100× below the downward rate |
