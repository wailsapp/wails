...
While streams are unreleased, point a test app at the branch's package:

```bash
npm install file:/path/to/wails/v3/internal/runtime/desktop/@wailsio/runtime
```

That package ships `dist/`, which is built by `npx tsc` in its directory — separate from
the esbuild step that produces `bundledassets/runtime.js` for the webview. Both need
rebuilding after a change to `stream.ts`: `task build:assets` for the bundle, `npx tsc` for
the npm package.
