...
**Frames transfer ownership rather than being copied.** Go `Send` retains the caller's
slice until the poll writes it, and JavaScript `send()` retains mutable array storage until
the POST is issued. Callers must not mutate or reuse that storage after handing it over.
Strings and freshly marshalled buffers are naturally safe. This deliberate difference from
a real WebSocket avoids a full-frame copy on both hot paths.
