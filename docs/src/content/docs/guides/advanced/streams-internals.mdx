...
| `streamMaxConnectionsGlobal` | 4,096 | live connections plus undelivered close notifications across the application |
...
Non-close controls therefore have their own bounded queue, while connection admission
reserves one close slot per accepted connection. When that capacity is occupied, a new
open receives retryable backpressure before it is registered.
...
and server transports. Live connections and pending close notifications share a 4,096
entry lifecycle budget. Reaching a shared allowance applies the same blocking `Send` or
non-blocking `TrySend` behaviour as reaching a local allowance, and every drain, receive,
close, failed write, and shutdown path returns its reservation.
...
session. Incomplete and retryable chunk sets therefore also share a 256 MiB
application-wide budget. Retained parts also share a 4,096-entry metadata allowance, so
tiny or empty chunks cannot grow maps and slice bookkeeping without approaching the byte
limit. A request that would cross either allowance receives retryable backpressure;
delivery, rejection, expiry, or session shutdown returns both bytes and part entries to
the shared budgets.
...
1. **A newer session poll for a window supersedes older generations.** A reload gives the
   ...
   `sessionStorage`. Every request carries both values. A poll retires only lower page
   generations, so server scheduling cannot make a delayed request from the previous page
   look newer than its replacement. The manager retains a per-window retired-generation
   watermark so an already in-flight request cannot recreate the old page without retaining
...
**The platform layer does not report a cancelled request.** The webview request context is
`context.Background()` with a deferred cancel (`assetserver_webview.go`), so a poll
belonging to a page that has navigated away stays parked until the hold expires. Rule 1
means the *connection* closes promptly regardless; it is the parked request that lingers.
Plumbing `stopURLSchemeTask` (and its Linux/Windows equivalents) through to a
`context.CancelFunc` would fix that, and it is the single most valuable outstanding item.
Note the same cancellation surfaces three different ways: a caught ObjC exception on macOS,
`EPIPE` on Linux, and nothing at all until `Finish` on Windows.
...
| | status |
|---|---|
| Request cancellation from the platform layer | **not done** — the highest-value item; see above |
