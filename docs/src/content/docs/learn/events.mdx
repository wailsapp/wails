...
Once the event is registered, data arguments passed to the `EmitEvent` method will be checked against the specified type.
...
When emitting a custom event, you can also check the value returned by the `EmitEvent` method (for the app and windows):

```go
if app.EmitEvent("myevent") {
	app.Logger.Info("Event was cancelled")
    return
}
```
