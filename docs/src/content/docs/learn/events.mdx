...
On the Go side, listen for the `events.Common.WindowDropZoneFilesDropped` event. This event will **only** be emitted when files are dropped onto an element that has the `data-wails-dropzone` attribute.
...
If the drop occurs on an element that is a child of an element with `data-wails-dropzone`, `ElementID` and `ClassList` will refer to the child element, but the event is still correctly filtered by the parent dropzone attribute.
