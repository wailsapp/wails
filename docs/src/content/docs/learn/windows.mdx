...
- **Windows**: Uses `WDA_EXCLUDEFROMCAPTURE` on Windows 10 version 2004+ (build 19041+). Falls back to `WDA_MONITOR` on older versions.
- **macOS**: Uses `NSWindowSharingType` to set the window as read-only for screen sharing.
