...
Wails v3 is beta software with a stable desktop API. Applications are running in
production with it, but you should test thoroughly before deployment while we
complete the final polish for 3.0. See the [project status page](/status) for
the current picture. Wails v2 is the current stable release and continues to
receive fixes.
