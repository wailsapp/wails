...
If multiple buttons are marked as default or cancel, the **last one wins**.
...
Appends a pre-configured button to the dialog.
...
**Example:**
```go
dialog.WithButton(application.Button{Label: "Retry", IsDefault: true})
```

#### AddButton()

Adds a button with the given label.
...
Adds a default button (activated by Enter). If a default button already exists, it will be cleared.
...
Adds a cancel button (activated by Escape). If a cancel button already exists, it will be cleared.
...
Marks the button with the given label as the default (activated by pressing Enter).

If multiple buttons match the label, the **last one wins**.
...
Marks the button with the given label as the cancel button (activated by pressing Escape).
...
Shows the dialog (blocking) and returns the clicked button label.
...
func (s *Service) OpenRecent(app *application.App, recentPath string) error {
    ...
    if _, err := os.Stat(recentPath); os.IsNotExist(err) {
        ...
        dialog.Buttons(
			application.Button{Label: "Remove"},
			application.Button{Label: "Cancel", IsDefault: true, IsCancel: true},
		)
