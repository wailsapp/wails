...
Emits a custom event to all windows. Returns `true` if a hook cancelled the emit.
...
Removes all listeners for one or more events. `Off` takes variadic event-name strings — it does **not** take a callback. To remove a single listener, keep the unsubscribe function returned by `Events.On(...)` and call it.
...
Removes **every** event listener. Takes no arguments.
...
Listens for an event up to `max` times, then automatically unsubscribes.
...
There is **no** generic "application shutdown" event constant — register shutdown cleanup via `application.Options.OnShutdown` or `app.OnShutdown(func())`.
...
Window event constants live in the `events` package: `events.Common.*` for cross-platform events (and `events.Mac.*` / `events.Windows.*` / `events.Linux.*` for platform-specific ones).
...
- `events.Common.WindowDidResize` - Window was resized.
- `events.Common.WindowDidMove` - Window was moved.
