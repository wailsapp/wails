...
Create a `*ContextMenu` via the manager and register it under a name. `ContextMenuManager.Add` takes `*ContextMenu` — **not** `*Menu`, and there is **no** `app.RegisterContextMenu` method.
