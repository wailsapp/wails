...
In v2, project configuration lived in `wails.json`, which controlled the build, dev server, and frontend commands.

In v3, the project build system is driven by **`Taskfile.yml`** — a task runner configuration that replaces the build logic previously embedded in `wails.json`. The `wails.json` file still exists for some metadata, but the build workflow is defined in `Taskfile.yml`. **Do not hand-edit `Taskfile.yml`** for a fresh project — run `wails3 init` to generate the correct structure instead.
...
**v3 (`Taskfile.yml`, generated by `wails3 init`):**
...
The recommended approach for migration is to create a fresh v3 project with `wails3 init` and copy your application code into it, rather than trying to convert the `wails.json` manually:
