...
:::tip[Automate the first pass]
The `wails3 migrate` command generates the v3 configuration, converts your `wails.Run` options into a v3 bootstrap, and writes a report of the remaining manual steps. See the [v2 vs v3 comparison](/migration/comparison) page for details and a side-by-side API reference.
:::
...
A: v3 is beta software with a stable desktop API. Applications are running in
