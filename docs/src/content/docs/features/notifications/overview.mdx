...
Notifications on macOS require user authorization. Request and check authorization using callbacks:

```go
// Check current authorization status
notifier.CheckNotificationAuthorization(func(authorized bool, err error) {
    if err != nil {
    ...
    if authorized {
        ...
        notifier.RequestNotificationAuthorization(func(authorized bool, err error) {
            if err != nil {
                // Handle authorization error
                return
            }
            if authorized {
                // User granted authorization, now you can send notifications
            } else {
...
On Windows and Linux, the callbacks are invoked immediately with `authorized = true` and `err = nil`.
