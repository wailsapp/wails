...
**Access:** `wails://localhost:9245/files/...` or programmatically within your frontend via `window.location.origin + '/files/...'` or `new URL('/files/...', window.location.origin).toString()`. The programmatic approach is preferable in case the port number is non-default.
