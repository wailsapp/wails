...
- Pass the concrete service pointer directly so the binding generator can identify its package and type. Do not first store it in an interface-typed variable.
