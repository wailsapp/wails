...
Use `AddButton(...)`/`AddDefaultButton(...)`/`AddCancelButton(...)` (or `Buttons(...)`) to define buttons. Use `Result()` to block and retrieve the clicked button's label:
...
result, err := app.Dialog.Question().
	SetMessage("Choose an action").
	AddDefaultButton("Option 1").
	AddButton("Option 2").
	AddCancelButton("Option 3").
	Result()
// ...
```
...
You can configure default/cancel behavior using `AddDefaultButton(...)` and `AddCancelButton(...)`. If multiple default/cancel buttons are specified, the **last one wins**.
...
func showCustomIconQuestion(app *application.App, iconBytes []byte) {
	result, _ := app.Dialog.Question().
		...
		SetMessage("Using a custom icon").
		SetIcon(iconBytes).
		AddDefaultButton("I like it!").
		AddButton("Not so keen...").
		Result()
    switch result {
    case "I like it!":
