...
There are two ways to create message dialogs:
...
**Via DialogManager (on the App instance):**
```go
app.Dialog.Info()
app.Dialog.Question()
app.Dialog.Warning()
...
Both approaches return a `*MessageDialog` that can be configured using method chaining.
...
```go
func showCustomIconQuestion(iconBytes []byte) {
    dialog := application.QuestionDialog().
        SetTitle("Custom Icon Example").
        SetMessage("Using a custom icon").
        SetIcon(iconBytes)
