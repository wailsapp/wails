...
if result, _ := dialog.Result(); result == "Delete" {
	deleteFile()
}
...
Use `Buttons(...)` to define buttons and `Result()` to retrieve the clicked button label:
...
Use `IsDefault` to specify which button is highlighted and triggered by Enter.
Use `IsCancel` to specify which button is triggered by Escape.
...
func exportData(app *application.App) {
    ...
    dialog.Buttons(
		application.Button{Label: "Export"},
		application.Button{Label: "Cancel", IsDefault: true, IsCancel: true},
	)
