...
<a href={wailsAppRegionVideo}>Open the video directly</a>
...
Be aware that this changes how the window hosts and interacts with WebView2 under the hood. Instead of the default HWND-hosted WebView2 controller, Wails uses composition controller hosting and forwards input explicitly. Treat `WebView2CompositionHosting` as an experimental mode: enable it only when you need native custom caption-button behavior, and test your app carefully on the Windows and WebView2 Runtime versions you support.
...
`WebView2CompositionHosting` uses WebView2 composition controller hosting with DirectComposition. This mode is experimental and may have rendering, input, focus, or WebView2 Runtime compatibility issues. Enable it only when you need native behavior for custom caption buttons, and test the result on the Windows versions and WebView2 Runtime versions you support.
:::
