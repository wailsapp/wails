...
**Platform support:**
- **macOS:** Solid backgrounds use public APIs. Transparent and translucent web content require the opt-in `privatemacapis` build tag.
...
- `MacBackdropLiquidGlass` - Native Liquid Glass on macOS 26+, with a translucent fallback on older releases

`MacBackdropLiquidGlass` uses public AppKit glass APIs by default. macOS does not
provide a public API that makes `WKWebView` itself transparent, so revealing a
native backdrop through transparent HTML requires the opt-in `privatemacapis`
build tag. The same tag is required for `MacBackdropTransparent` and
`MacBackdropTranslucent` to reveal content behind the webview.

The Go API is identical in both modes. Enable the complete macOS behavior when
developing, building, or packaging with:

```bash
# Development
EXTRA_TAGS=privatemacapis wails3 dev

# Production build
wails3 build -tags privatemacapis

# Package through the generated Taskfile
wails3 package EXTRA_TAGS=privatemacapis GOOS=darwin
```

<Aside type="caution">
The `privatemacapis` build tag enables undocumented WebKit and AppKit APIs. Do
not use it for Mac App Store submissions. Without the tag, Wails uses the public
`WKWebView.inspectable` API on macOS 13.3+, so a window can still be inspected
manually from Safari's Develop menu; automatic inspector opening is disabled.
</Aside>

`MacLiquidGlass.Style`, `CornerRadius`, and `TintColor` remain available in
public builds. `Automatic`, `Light`, and `Dark` use the public regular glass
style with system appearance selection, while `Vibrant` uses the public clear
style. Cross-window `GroupID` and `GroupSpacing` have no public AppKit
equivalent and are active only with `privatemacapis`.
