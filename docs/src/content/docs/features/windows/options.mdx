...
Wails panels remain visible when the application deactivates and are released
when closed, matching the lifecycle expected by `WebviewWindow`. These are
intentional overrides of the opposite `NSPanel` defaults.

Window class, level, activation policy, and collection behaviour solve separate
problems:
...
An explicit `WindowLevel` takes precedence over `AlwaysOnTop` and
`PanelPreferences.FloatingPanel`. Without an explicit level, `AlwaysOnTop` or a
floating panel resolves to `MacWindowLevelFloating`; otherwise the level is
`MacWindowLevelNormal`. Calling `SetAlwaysOnTop` later remains an explicit
runtime change.
