...
**Platform support:**
- **macOS:** Transparent and translucent web content uses private WebKit APIs by default. With `noprivateapis`, the native window effect remains configured but the webview is opaque.
...
macOS does not provide a public API that makes `WKWebView` itself transparent.
Wails therefore uses private WebKit APIs by default for
`MacBackdropTransparent`, `MacBackdropTranslucent`, and full-window
`MacBackdropLiquidGlass`.

The Go API is identical in both modes. To exclude Wails' known private macOS
APIs, build with:
...
<Aside type="caution">
Default macOS builds contain private WebKit and AppKit API references and may be
unsuitable for Mac App Store submission. Use `noprivateapis` when private APIs
are prohibited. See [macOS Packaging](/guides/build/macos#private-macos-apis)
for the complete fallback table.
</Aside>

With `noprivateapis`, `Automatic`, `Light`, and `Dark` use the public regular
glass style, while `Vibrant` uses the public clear style. `CornerRadius` and
`TintColor` remain active. Cross-window `GroupID` and `GroupSpacing` have no
public AppKit equivalent and are ignored.
