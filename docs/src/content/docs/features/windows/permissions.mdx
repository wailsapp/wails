...
This works without any `Permissions` configuration. When the map is set, macOS honours it for camera and microphone requests from web content: `PermissionDeny` refuses the request before the system prompt, `PermissionAllow` grants it within whatever the user has already allowed at the system level, and `PermissionPrompt` (the default) lets the system prompt appear. Geolocation and notifications still go through the system privacy framework regardless of the map. To request or inspect system-level permissions from Go, use `app.Permissions`, described in the macOS Platform Integration guide.
