...
**Best practice:** Use `Close()` for user-initiated closes, `Destroy()` for cleanup. To prevent closing, use `RegisterHook(events.Common.WindowClosing, ...)` and call `event.Cancel()`.
