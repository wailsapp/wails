...
- Отображаются обновления интерфейса
...
Сервисы — это основной способ управления жизненным циклом в Wails v3. Они предоставляют хуки запуска и завершения через интерфейсы. Полную документацию по сервисам см. в [руководстве по сервисам](/ru/features/bindings/services).
...
Колбэк `ShouldQuit` вызывается всякий раз, когда запрашивается выход — будь то пользователь, закрывший последнее окно, нажавший Cmd+Q (macOS) / Alt+F4 (Windows) или вызвавший `app.Quit()` программно.
...
macOS следует нативным соглашениям платформы, где приложения обычно остаются активными в строке меню даже при отсутствии окон. Windows и Linux по умолчанию завершают работу.
...
Это полезно для приложений в системном трее или приложений, которые должны оставаться работающими в фоновом режиме.
...
func (s *DatabaseService) ServiceStartup(ctx context.Context, options application.ServiceOptions) error {
    ...
    return nil
}**Вопросы о жизненном цикле?** Задавайте их в [Discord](https://discord.gg/JDdSxwjhGf) или проверяйте [примеры](https://github.com/wailsapp/wails/tree/master/v3/examples).