...
API менеджера Wails v3 предоставляет организованный и удобный способ доступа к функциональности приложения через специализированные структуры менеджеров. Эта новая структура API группирует связанные методы вместе, сохраняя при этом полную обратную совместимость с традиционным API приложения.

## Обзор

API менеджера организует функциональность приложения по одиннадцати основным направлениям:

- **`app.Window`** - Создание, управление и обратные вызовы окон
- **`app.ContextMenu`** - Регистрация и управление контекстным меню
- **`app.KeyBinding`** - Управление глобальными сочетаниями клавиш
- **`app.Browser`** - Интеграция с браузером (открытие URL и файлов)
- **`app.Env`** - Информация об окружении и состояние системы
- **`app.Dialog`** - Операции с диалогами файлов и сообщений
...
Управляет созданием окон, их получением и обратными вызовами жизненного цикла.
...
app.Event.Reset() // Удаление всех слушателей
...
Обеспечивает интеграцию с браузером для открытия URL и файлов.

```go
// Открытие URL и файлов в браузере по умолчанию
...
Доступ к информации о системном окружении.
...
Организованный доступ к диалогам файлов и сообщений.
...
// Уничтожение системного трея при завершении работы
systray.Destroy()
```