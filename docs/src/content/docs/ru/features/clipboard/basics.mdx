...
description: Копирование и вставка текста с помощью системного буфера обмена
sidebar:
  order: 1
---
...
Wails предоставляет **единый API буфера обмена**, который работает на всех платформах. Копируйте и вставляйте текст с помощью простых и согласованных методов в Windows, macOS и Linux.
...
**Возвращает:** `(string, bool)` — Текст и флаг успеха
...
Для простого текста вы можете использовать API буфера обмена браузера:
...
**Примечание:** API буфера обмена браузера требует HTTPS или localhost, а также разрешения пользователя.
...
- **Проверяйте возвращаемые значения** — обрабатывайте ошибки gracefully---

**Вопросы?** Задавайте их в [Discord](https://discord.gg/JDdSxwjhGf) или смотрите [примеры работы с буфером обмена](https://github.com/wailsapp/wails/tree/master/v3/examples).