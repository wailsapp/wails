   ...
   <Tabs syncKey="os">
     ...
     <TabItem label="Linux" icon="linux">
       **Build tools and WebKit**
       ...
       Wails v3 requires **WebKitGTK 4.1** (uses libsoup3). Older releases that only ship 4.0 — Ubuntu 20.04, Debian 11, RHEL 8/9 and their derivatives — are not supported.
       :::
       ...
       <Tabs syncKey="distro">
         ...
         <TabItem label="openSUSE">
           ```bash
           sudo zypper install gcc pkg-config gtk3-devel webkit2gtk4_1-devel
           ```
         </TabItem>
         ...
         <TabItem label="NixOS">
           ...
           ```nix
           buildInputs = [ pkgs.webkitgtk_4_1 pkgs.gtk3 pkgs.pkg-config ];
           ```
         </TabItem>
