...
1. **Generate the project**
   ...
   :::tip[Other Templates]
   Try `-t react`, `-t vue`, or `-t svelte` for your preferred framework. These are
   TypeScript by default; for plain JavaScript use `-t vanilla-js` or `-t react-js`.
   Run `wails3 init -l` to see all available templates, or
   [bring your own frontend framework](/guides/dev/frontend-frameworks).
   :::
