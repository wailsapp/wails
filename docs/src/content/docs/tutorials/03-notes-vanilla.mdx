...
2. **Create the NotesService**
   ...
   func (n *NotesService) SaveToFile() error {
   	...
   	if err := application.Get().Dialog.Info().
   		SetTitle("Success").
   ...
   **What's happening here:**
   ...
   - **File dialogs**: Uses `application.Get().Dialog.SaveFile()` and `application.Get().Dialog.OpenFile()`
...
In Wails v3, dialogs are accessed via the app's dialog manager:
...
if err := app.Dialog.Info().SetMessage("Saved!").Show(); err != nil {
	return err
}
```
