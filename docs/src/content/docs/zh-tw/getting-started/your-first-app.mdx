...
本指南將向你展示如何建立你的第一個 Wails v3 應用程式，涵蓋專案設定、建置以及開發工作流程。
...
2.  ## 探索專案結構
    ...
    :::note
    雖然 Wails v3 使用 [Task](https://taskfile.dev/) 作為預設的建置系統，但你也可以使用 `make` 或其他替代的建置系統。
    :::
...
3.  ## 建置你的應用程式
    ...
    :::note
    `wails3 build` 是 `wails3 task build` 的縮寫，它會執行 `Taskfile.yml` 中的 `build` 任務。
    :::
    ...
    你將看到一個簡單的介面，這是你的應用程式的起點。由於這是除錯版本，你也會在控制台視窗中看到記錄。這對於除錯目的很有用。

4.  ## 開發模式

    我們也可以以開發模式執行應用程式。此模式允許你修改前端程式碼，並在不重新建置整個應用程式的情況下，即時看到變更反映在執行的應用程式中。
...
5.  ## 封裝你的應用程式
    ...
    <TabItem label="Mac" icon="apple">
              ...
              這將建立一個生產環境建置，並將其封裝到 `bin` 目錄中的 `.app` 套件裡。
            ...
            <TabItem label="Windows" icon="seti:windows">
              ...
              這將建立一個生產環境建置，並將其封裝到 `bin` 目錄中的 NSIS 安裝程式裡。
        ...
        有關封裝選項和設定的更多詳細資訊，請查看我們的 [封裝指南](/guides/packaging)。

6.  ## 設定版本控制與模組名稱

    你的專案已建立，但模組名稱為佔位符 `changeme`。建議將其更新以匹配你的儲存庫 URL：
    ...
    3. 設定你的遠端儲存庫（替換為你的儲存庫 URL）：
       ...
       ```
    4. 更新 `go.mod` 中的模組名稱以匹配你的儲存庫 URL：
       ```bash
...
您剛剛建立、開發並封裝了您的第一個 Wails 應用程式。
這只是您使用 Wails v3 所能達成成就的開始。

## 下一步

如果您是 Wails 的新手，我們建議您接著閱讀我們的教學課程，這將是關於 Wails 各種功能的實用指南。第一個教學課程是 [建立服務](/zh-tw/tutorials/01-creating-a-service)。
