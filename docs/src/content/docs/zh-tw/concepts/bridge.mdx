...
**關鍵洞察：** 無需 HTTP、無需 IPC、無需程序邊界。僅有**直接函數呼叫**與**類型安全**。

## 運作原理：逐步說明

### 1. 服務註冊（啟動階段）

當應用程式啟動時，Wails 會掃描您的服務：
...
2. **提取類型資訊**（參數、返回類型）
3. **建立註冊表**，將方法名稱映射到函數
4. **生成 TypeScript 綁定**，包含完整的類型定義

### 2. 綁定生成（編譯時期）

Wails 會自動生成 TypeScript 綁定：
...
**發生的過程：**
1. **呼叫綁定函數** - `Greet("World")`
2. **建立訊息** - `{ service: "GreetService", method: "Greet", args: ["World"] }`
3. **傳送至橋接器** - 透過 WebView 的 JavaScript 橋接
4. **返回 Promise** - 等待回應
...
**零拷貝最佳化：** 大型資料（&gt;1MB）在可能的情況下使用共享記憶體。
...
**對橋接器有疑問？** 在 [Discord](https://discord.gg/JDdSxwjhGf) 提問或查看 [綁定範例](https://github.com/wailsapp/wails/tree/master/v3/examples/binding)。