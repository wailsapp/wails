---
title: 你的第一個應用程式
description: 在 10 分鐘內建立一個可運行的 Wails 應用程式
sidebar:
  order: 3
---
...
建議使用 [Dev Drive](https://learn.microsoft.com/en-us/windows/dev-drive/) 來儲存你的專案。Dev Drives 針對開發者工作負載進行了最佳化，與一般的 NTFS 磁碟機相比，可以將建置時間和磁碟存取速度提高高達 30%。
:::
...
3. **執行應用程式**
   ...
   :::note[首次執行]
   首次執行可能需要比預期更長的時間，因為它會安裝前端相依套件、產生繫結等。後續的執行速度會快得多。
   :::
...
可以將服務視為你的後端 API，而將繫結視為與之通訊的用戶端程式庫。
:::

### 註冊服務

開啟 `main.go` 並找到服務註冊程式碼：
...
<Tabs syncKey="os">
  <TabItem label="Windows" icon="seti:windows">
    **輸出：** `build/bin/myapp.exe`
    ...
    雙擊即可執行。不需要相依套件（WebView2 是 Windows 的一部分）。
  </TabItem>
...
:::tip[跨平台建置]
想要為其他平台建置？請參閱 [跨平台建置 →](/zh-tw/guides/build/cross-platform)
:::
