   ...
   <Tabs syncKey="os">
     ...
     <TabItem label="Linux" icon="linux">
       **建置工具和 WebKit**
       ...
       :::note[選用：GTK4 支援]
       Wails 也有實驗性的 GTK4 / WebKitGTK 6.0 支援。如果你想使用 GTK4 建置，請安裝你發行版的額外相依套件，並使用 `wails3 build -tags gtk4` 進行建置。請參閱 [Linux 封裝 - GTK4](/guides/build/linux#gtk4-support-experimental) 以獲取詳細資訊。
       :::
     </TabItem>
   </Tabs>
...
3. **安裝 Wails CLI**
   ...
   這會將 `wails3` 命令安裝到 `~/go/bin`（或在 Windows 上為 `%USERPROFILE%\go\bin`）。
...
4. **驗證安裝**
   ...
   **預期輸出（或類似）：**
   ...
   你的 `~/go/bin` 不在 PATH 中。請參閱上面的步驟 1 來修復此問題，然後重新啟動終端機。
   :::

5. **安裝 npm（選用但建議）**

   大多數 Wails 範本使用 npm 進行前端工具鏈。
...
想要使用主開發分支中最新的程式碼嗎？這讓您在功能發布前就能體驗新功能與修復，但伴隨的風險是可能存在錯誤或破壞性變更。僅建議貢獻者或需要測試即將推出功能的人使用。
...
**遇到問題嗎？** 請在 [Discord](https://discord.gg/JDdSxwjhGf) 提問或[開啟問題](https://github.com/wailsapp/wails/issues)。