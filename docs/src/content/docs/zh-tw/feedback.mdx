...
<Tabs>
  <TabItem label="錯誤" icon="error">
    ...
    - 文章應清楚說明錯誤內容，並提供簡單的複現範例。如果文件不清楚說明 *應該* 發生的情況，請在文章中包含該資訊。
    - 文章應加上 `Bug` 標籤。
    - 請在文章中包含 `wails doctor` 的輸出結果。
    ...
    _請記住_，非預期的行為不一定是錯誤——它可能只是沒有按照您的預期運作。請使用 `Suggestions` 標籤來反映此類情況。
  ...
  <TabItem label="按讚" icon="star">
    ...
    - 請 *不要* 僅添加 "+1" 或 "me too" 等評論。
    - 如果文章還有其他內容可以補充，例如「此錯誤也影響 ARM 構建」或「另一個選項是....」，歡迎隨時留言。
