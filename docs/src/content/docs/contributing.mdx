...
Tests are part of the change, not a final smoke check. Keep unit tests next to
the code they exercise and prefer table-driven tests whenever one behaviour is
being checked with several inputs or edge cases. Name each case so a failure
explains the scenario.

New and changed logic should be fully covered. Aim for 100% Go statement
coverage of the code your PR adds or changes; do not treat a repository-wide
percentage as a substitute for testing the change. A gap can be valid—for
example, an OS-only error path or a condition that is impractical to reproduce
without real hardware—but explain the gap and why it cannot reasonably be
...
For integration suites, race detection, and the full CI-equivalent commands,
see [Testing & Continuous Integration](/contributing/testing-ci/).
