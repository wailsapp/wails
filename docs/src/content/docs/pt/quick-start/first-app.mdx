...
Considere usar o [Dev Drive](https://learn.microsoft.com/en-us/windows/dev-drive/) para armazenar seus projetos. Os Dev Drives são otimizados para cargas de trabalho de desenvolvedores e podem melhorar significativamente os tempos de build e as velocidades de acesso ao disco em até 30% em comparação com drives NTFS comuns.
:::
...
1. **Gere o projeto**
   ...
   Isso cria um novo projeto com o modelo padrão Vanilla + Vite (HTML/CSS/JS puro com bundler Vite).
...
2. **Método exportado** - `Greet` é capitalizado, tornando-o disponível para o frontend
3. **Lógica simples** - Recebe um nome e retorna uma saudação
4. **Tipagem segura** - Os tipos de entrada e saída são definidos

:::tip[Entendendo Serviços e Bindings]
**Serviços** são módulos Go autônomos que expõem funcionalidade para o seu frontend. Eles são apenas structs Go comuns com métodos exportados que você registra no campo `Services` da configuração da sua aplicação.

**Bindings** são o SDK TypeScript/JavaScript gerado automaticamente que permite ao seu frontend chamar esses serviços. Quando você executa `wails3 dev` ou `wails3 build`, o Wails analisa seus serviços registrados e gera bindings com tipagem segura em `frontend/bindings/`.

Pense nos serviços como sua API backend e nos bindings como a biblioteca cliente que se comunica com ela.
:::

### Registrando o Serviço

Abra `main.go` e encontre o registro do serviço:
...
2. **O app será reconstruído automaticamente**
   ...
   Alterações no código Go acionam uma reconstrução automática e reinício. Alterações no frontend usam hot-reload sem reinício.
   :::
...
<Tabs syncKey="os">
  ...
  <TabItem label="macOS" icon="apple">
    **Saída:** `build/bin/myapp.app`
    ...
    Arraste para a pasta Aplicações ou clique duas vezes para executar.
  </TabItem>
...
:::tip[Builds Cross-Platform]
Quer compilar para outras plataformas? Veja [Builds Cross-Platform →](/pt/guides/build/cross-platform)
:::
...
- Alterações no frontend usam hot-reload instantaneamente
