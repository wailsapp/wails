   ...
   <Tabs syncKey="os">
     <TabItem label="Windows" icon="seti:windows">
       ...
       **Verifique o PATH:**
       ...
       Se estiver vazio, adicione `C:\Users\SeuNome\go\bin` ao seu PATH.
     </TabItem>
...
2. **Instale as Dependências da Plataforma**
   ...
   <Tabs syncKey="os">
     ...
     <TabItem label="macOS" icon="apple">
       ...
       **Isso é tudo!** O macOS inclui o WebKit por padrão.
     </TabItem>
     ...
     <TabItem label="Linux" icon="linux">
       **Ferramentas de compilação e WebKit**
       ...
       O Wails v3 requer **WebKitGTK 4.1** (usa libsoup3). Lançamentos mais antigos que apenas disponibilizam a versão 4.0 — Ubuntu 20.04, Debian 11, RHEL 8/9 e seus derivados — não são suportados.
       :::
       ...
       :::note[Opcional: Suporte a GTK4]
       O Wails também possui suporte experimental ao GTK4 / WebKitGTK 6.0. Se você quiser compilar com GTK4, instale as dependências adicionais para sua distribuição e compile com `wails3 build -tags gtk4`. Veja [Empacotamento no Linux - GTK4](/guides/build/linux#gtk4-support-experimental) para mais detalhes.
       :::
     </TabItem>
   </Tabs>
...
<Tabs syncKey="os">
  ...
  <TabItem label="Linux" icon="linux">
    Baixe o arquivo tar mais recente em [go.dev/dl](https://go.dev/dl/), depois:
    ```bash
    sudo rm -rf /usr/local/go
    sudo tar -C /usr/local -xzf go1.25.linux-amd64.tar.gz
    ```
