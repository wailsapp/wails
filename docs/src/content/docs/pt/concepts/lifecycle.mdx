...
Aplicações de desktop possuem um ciclo de vida que vai da inicialização ao encerramento. O Wails v3 fornece **serviços**, **eventos** e **hooks** para gerenciar esse ciclo de forma eficaz.
...
3. O servidor de ativos é configurado
...
Os serviços são a maneira principal de gerenciar o ciclo de vida no Wails v3. Eles fornecem hooks de inicialização e encerramento por meio de interfaces. Para documentação completa sobre serviços, consulte o [Guia de Serviços](/pt/features/bindings/services).
...
Para um gerenciamento de ciclo de vida mais complexo com lógica de inicialização, injeção de dependência ou recursos com estado, use [Serviços](#services-lifecycle) em vez disso.
...
**Valor de retorno:**
- Retorne `true` para permitir que o encerramento prossiga (a aplicação será encerrada)
...
Se `ShouldQuit` não estiver definido, a aplicação será encerrada imediatamente quando solicitada.
...
- O processo é encerrado forçadamente (SIGKILL, Gerenciador de Tarefas forçar encerramento)
- `os.Exit()` é chamado diretamente

### OnShutdown

O callback `OnShutdown` é chamado quando a aplicação é confirmada para estar encerrando (após `ShouldQuit` retornar `true`, se definido). Use isso para tarefas de limpeza, como salvar o estado, fechar conexões de banco de dados ou liberar recursos.
...
app := application.New(application.Options{
    OnShutdown: func() {
        ...
        // Close connections----------|--------------------------------|
| macOS | O app continua em execução (a barra de menu permanece) |
| Windows | O app encerra |
| Linux | O app encerra |
...
**Fazer todas as plataformas encerrarem quando a última janela for fechada:**
...
**Fazer todas as plataformas permanecerem em execução quando a última janela for fechada:**

Isso é útil para aplicações de bandeja do sistema ou apps que devem permanecer em execução em segundo plano.
