...
O Wails é um framework para construir aplicações de desktop usando Go e tecnologias web. Ele oferece integração nativa com o sistema operacional, permitindo que você construa sua interface do usuário com HTML, CSS e JavaScript.
...
Sim! O Wails v3 é adequado para aplicações de produção. Muitas empresas usam o Wails para suas aplicações de desktop.
...
Sua aplicação estará em `build/bin/`.
...
Veja o [Guia de Instaladores](/pt/guides/installers).

### Como assino digitalmente minha aplicação?
...
Sim! O Wails v3 tem suporte nativo a múltiplas janelas:
...
Sim! Crie aplicações para a bandeja do sistema:
...
Veja o [Guia de Desempenho](/pt/guides/performance).
...
Veja o [Guia de Migração](/pt/migration/v2-to-v3).
...
Veja o [Guia de Contribuição](/pt/contributing).
