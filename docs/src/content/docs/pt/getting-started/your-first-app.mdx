...
Este guia mostra como criar seu primeiro aplicativo Wails v3, cobrindo a configuração do projeto, compilação e fluxo de trabalho de desenvolvimento.
...
2.  ## Explorando a Estrutura do Projeto
    ...
    - build/           Contém arquivos usados pelo processo de compilação
        ...
        - darwin/      Arquivos de compilação específicos do macOS
            ...
            - icons.icns    Ícone do aplicativo do macOS
        ...
        - windows/     Arquivos de compilação específicos do Windows
            ...
            - icon.ico           Ícone do aplicativo do Windows
...
4.  ## Modo de Desenvolvimento
    ...
    faça alterações no código do frontend e veja as alterações refletidas no
...
5.  ## Empacotando Seu Aplicativo
    ...
    <TabItem label="Mac" icon="apple">
            ...
            <TabItem label="Linux" icon="linux">
              ...
              ```bash
              # Crie todos os tipos de pacote (AppImage, deb, rpm e Arch Linux)
              wails3 package

              # Ou crie tipos de pacote específicos
...
6.  ## Configurando Controle de Versão e Nome do Módulo
    ...
    3. Defina seu repositório remoto (substitua pela URL do seu repositório):
       ...
       ```
    4. Atualize o nome do seu módulo em `go.mod` para corresponder à URL do seu repositório:
       ```bash
    ...
    Isso garante que o nome do seu módulo Go esteja alinhado com as convenções de nomenclatura de módulos do Go e facilita o compartilhamento do seu código.:::tip[Dica]
...
```
Isso suporta vários formatos de URL Git:
...
Este é apenas o começo do que você pode alcançar com o Wails v3.

## Próximos Passos

Se você é novo no Wails, recomendamos ler nossos Tutoriais a seguir, que serão um guia prático através
das várias funcionalidades do Wails. O primeiro tutorial é [Criando um Serviço](/pt/tutorials/01-creating-a-service).
