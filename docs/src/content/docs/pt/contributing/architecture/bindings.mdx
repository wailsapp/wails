...
4. Ele identifica tipos de modelo (structs usadas como parâmetros ou valores de retorno em métodos de serviço)
...
4. O renderer aplica quaisquer injeções de código personalizadas especificadas pelas diretivas `//wails:inject`
...
1. Durante a fase de coleta, o collector identifica e analisa as diretivas no código Go
...
O sistema de bindings suporta geração condicional de código usando um prefixo de condição de dois caracteres para as diretivas `include` e `inject`:
...
O gerador de bindings é projetado para ser eficiente, mas há algumas coisas a ter em mente:
