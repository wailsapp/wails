...
description: Daftarkan pintasan keyboard global untuk akses cepat
sidebar:
  order: 1
---
...
Wails menyediakan sistem key binding yang powerful yang memungkinkan Anda mendaftarkan pintasan keyboard global yang bekerja di semua window aplikasi. Ini memungkinkan pengguna mengakses fungsionalitas dengan cepat tanpa navigasi menu.

## Mengakses Key Binding Manager

Key binding manager diakses melalui properti `KeyBinding` pada instance aplikasi Anda:
...
## Mengelola Key Binding

### Removing Key Bindings

Hapus key binding saat tidak diperlukan lagi:
...
Ambil semua key binding yang terdaftar:
...
Buat key binding context-aware dengan memeriksa state aplikasi:
...
Key binding menerima window aktif, memungkinkan perilaku spesifik window:
...
Tambah dan hapus key binding secara dinamis berdasarkan state aplikasi:
