---
title: Environment
description: Akses informasi environment sistem di aplikasi Wails Anda
sidebar:
  order: 59
---
...
Wails menyediakan informasi environment komprehensif melalui API EnvironmentManager. Ini memungkinkan aplikasi Anda mendeteksi properti sistem, preferensi tema, dan terintegrasi dengan file manager sistem operasi.

## Accessing the Environment Manager

Environment manager diakses melalui properti `Env` pada instance aplikasi Anda:
...
Ambil informasi komprehensif tentang environment runtime:
...
Deteksi apakah sistem menggunakan dark mode:
...
Dengarkan perubahan tema untuk memperbarui aplikasi secara dinamis:
...
Buka file manager sistem di lokasi tertentu:
...
Gunakan informasi environment untuk menyesuaikan perilaku aplikasi:
...
Gunakan informasi mode debug untuk mengaktifkan fitur development:
...
Buat dialog yang menampilkan informasi environment:
