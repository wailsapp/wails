---
title: Referensi CLI
description: Referensi lengkap perintah CLI Wails
sidebar:
  order: 1
---

CLI Wails menyediakan set perintah komprehensif untuk membantu Anda develop, build, dan maintain aplikasi Wails.

## Perintah Inti

Perintah inti adalah perintah utama untuk pembuatan proyek, development, dan build.

All CLI commands are of the following format: `wails3 <command>`.

### `init`
Inisialisasi proyek Wails baru. During this initialization, the `go mod tidy` command is run to bring the project packages up to date. This can be bypassed by using the `-skipgomodtidy` flag with the `init` command.
...
### `dev`
Jalankan aplikasi dalam mode development. This will give you a live view of your frontend code, and you can make changes and see them reflected
...
### `build`
Build versi debug aplikasi Anda. It defaults to building for the current platform and architecture.
...
Buat package spesifik platform untuk distribusi.
...
### `task`
Jalankan task yang didefinisikan di Taskfile.yml proyek Anda. This is an embedded version of [Taskfile](https://taskfile.dev) that allows you to define and run custom build, test, and deployment tasks.
