...
1.  ## Création d'un nouveau projet
    ...
    Cette commande crée un nouveau répertoire nommé `myfirstapp` avec tous les fichiers nécessaires.
...
2.  ## Exploration de la structure du projet
    ...
    <FileTree>
    - build/           Contient les fichiers utilisés par le processus de build
        - appicon.png  L'icône de l'application
        - config.yml   Configuration du build
        - Taskfile.yml Tâches de build
        - darwin/      Fichiers de build spécifiques à macOS
            - Info.dev.plist Configuration de développement
            - Info.plist    Configuration de production
            - Taskfile.yml  Tâches de build macOS
            - icons.icns    Icône de l'application macOS
        - linux/       Fichiers de build spécifiques à Linux
            - Taskfile.yml  Tâches de build Linux
            - appimage/     Packaging AppImage
                - build.sh  Script de build AppImage
            - nfpm/        Packaging NFPM
                - nfpm.yaml Configuration du package
                - scripts/  Scripts de build
        - windows/     Fichiers de build spécifiques à Windows
            - Taskfile.yml        Tâches de build Windows
    ...
    Prenez un moment pour explorer ces fichiers et vous familiariser avec la
    structure.

    :::note
    Bien que Wails v3 utilise [Task](https://taskfile.dev/) comme système de build par défaut, rien ne vous empêche d'utiliser `make` ou tout autre
    système de build alternatif.
    :::
...
4.  ## Mode développement
    ...
    Nous pouvons également exécuter l'application en mode développement. Ce mode vous permet de
    modifier votre code frontend et de voir les changements reflétés dans l'application
    en cours d'exécution sans avoir à recompiler l'ensemble de l'application.
    ...
    2. Modifiez la ligne contenant `return "Hello " + name + "!"` par
...
5.  ## Packaging de votre application
        ...
        Une fois votre application prête pour la distribution, vous pouvez créer
        des packages spécifiques à chaque plateforme :
    ...
    <TabItem label="Mac" icon="apple">
              ...
              Cela créera une build de production et la packagera dans un bundle `.app` dans le répertoire `bin`.
            ...
            <TabItem label="Windows" icon="seti:windows">
              ...
              Cela créera une build de production et la packagera dans un installateur NSIS dans le répertoire `bin`.
        ...
        Pour plus d'informations détaillées sur les options de packaging et la configuration,
        consultez notre [Guide de Packaging](/guides/packaging).
...
6.  ## Configuration du contrôle de version et du nom du module
    ...
    Cela garantit que le nom de votre module Go est aligné avec les conventions de nommage des modules Go et facilite le partage de votre code.:::tip[Conseil]
...
```
Cela prend en charge divers formats d'URL Git :
...
Vous venez de créer, développer et packager votre première application Wails.
Ce n'est que le début de ce que vous pouvez accomplir avec Wails v3.

## Prochaines étapes

Si vous débutez avec Wails, nous vous recommandons de consulter nos Tutoriels qui vous guideront de manière pratique à travers les différentes fonctionnalités de Wails. Le premier tutoriel est [Créer un Service](/fr/tutorials/01-creating-a-service).

Si vous êtes un utilisateur avancé, consultez nos [Guides](/guides) pour obtenir des informations plus détaillées sur l'utilisation de Wails.