---
title: Retour sur la version Alpha v3
description: Comment fournir des commentaires et signaler des problèmes pour Wails v3
sidebar:
  order: 40
---
...
messages existants avant d'en créer de nouveaux. Voici les différentes façons de fournir des commentaires :
...
<Tabs>
  <TabItem label="Bugs" icon="error">
    ...
    - Le message doit clairement indiquer quel est le bug et inclure un exemple simple reproductible. Si la documentation n'est pas claire sur ce qui *devrait* se produire, veuillez l'inclure dans le message.
    - Le message doit être tagué avec `Bug`.
    - Veuillez inclure la sortie de `wails doctor` dans votre message.
    - Si le bug correspond à un comportement qui ne s'aligne pas avec la documentation actuelle, par exemple une fenêtre qui ne redimensionne pas correctement, veuillez faire ce qui suit :
      ...
      - Veuillez inclure un lien vers la PR dans votre message.
  ...
  <TabItem label="Corrections" icon="approve-check">
    ...
    - Créer un message dans le canal [v3 Alpha Feedback](https://discord.gg/bdj28QNHmT).
    - Le message doit être tagué avec `PR`.
    - Veuillez inclure un lien vers la PR dans votre message.
  ...
  <TabItem label="Suggestions" icon="puzzle" id="abc">
    ...
    - Le message doit être tagué avec `Suggestion`.
  ...
  <TabItem label="Votes positifs" icon="star">
    ...
    - Les messages peuvent être "upvotés" en utilisant l'emoji :thumbsup: Veuillez voter pour les messages qui sont une priorité pour vous.
    - Veuillez *ne pas* simplement ajouter des commentaires comme "+1" ou "moi aussi".
    - N'hésitez pas à commenter s'il y a plus à ajouter au message, comme "ce bug affecte également les builds ARM" ou "Une autre option serait de ...."
