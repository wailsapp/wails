...
Wails fournit un **pont direct en mémoire** entre Go et JavaScript, permettant une communication fluide sans surcharge HTTP, sans limites de processus ni goulots d'étranglement liés à la sérialisation.
...
**Optimisation sans copie :** Les grandes données (&gt;1MB) utilisent la mémoire partagée lorsque cela est possible.
...
**Des questions sur le pont ?** Posez-les sur [Discord](https://discord.gg/JDdSxwjhGf) ou consultez les [exemples de liaison](https://github.com/wailsapp/wails/tree/master/v3/examples/binding).