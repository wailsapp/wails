...
description: Structure d'API organisée avec des interfaces de gestionnaire ciblées
sidebar:
  order: 2
---
...
L'API des gestionnaires de Wails v3 offre un moyen organisé et découvrable d'accéder aux fonctionnalités de l'application via des structs de gestionnaire ciblées. Cette nouvelle structure d'API regroupe les méthodes connexes tout en maintenant une compatibilité ascendante totale avec l'API App traditionnelle.

## Aperçu

L'API des gestionnaires organise les fonctionnalités de l'application en onze domaines ciblés :

- **`app.Window`** - Création, gestion et rappels de fenêtre
- **`app.ContextMenu`** - Enregistrement et gestion du menu contextuel
...
- **`app.Dialog`** - Opérations de dialogue de fichiers et de messages
...
- **Organisation du code améliorée** - Méthodes connexes regroupées ensemble
- **Maintenabilité améliorée** - Séparation des responsabilités entre les gestionnaires
...
Accès organisé aux dialogues de fichiers et de messages.
...
Opérations du presse-papiers pour la lecture et l'écriture de texte.
