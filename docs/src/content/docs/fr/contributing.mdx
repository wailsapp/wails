...
# Compiler l'interface de ligne de commande Wails
cd v3/cmd/wails3
go build
...
git commit -m "docs: améliorer la documentation des liaisons"
...
# Référencer les tickets associés
```
...
- [ ] Tous les tests passent
- [ ] Pas de modifications incompatibles (ou documentées)
...
// Il retourne une erreur si les données sont invalides.
    ...
    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            ...
            if (err != nil) != tt.wantErr {
                t.Errorf("ProcessData() erreur = %v, wantErr %v", err, tt.wantErr)
                return
            }
            if got != tt.want {
...
<CardGrid>
  ...
  <Card title="Documentation" icon="open-book">
    ...
    [Parcourir la documentation →](/fr/quick-start/why-wails)
  </Card>
</CardGrid>