...
Wails는 Go 코드를 컴파일하고, 프론트엔드 애셋을 번들링하며, 모든 것을 단일 실행 파일에 임베드하고, 플랫폼별 빌드를 처리하는 **통합 빌드 시스템**을 제공합니다. 모든 과정은 하나의 명령어로 완료됩니다.

```bash
wails3 build
```

**출력:** 모든 것이 임베드된 네이티브 실행 파일.
