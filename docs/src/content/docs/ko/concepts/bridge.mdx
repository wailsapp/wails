...
**핵심 통찰:** HTTP 없음, IPC 없음, 프로세스 경계 없음. **타입 안전성**을 갖춘 **직접 함수 호출**만 있습니다.

## 작동 원리: 단계별 설명

### 1. 서비스 등록 (시작 시)

애플리케이션이 시작될 때 Wails는 서비스들을 스캔합니다:
...
4. **전체 타입 정의가 포함된 TypeScript 바인딩 생성**
...
- **Wails Bridge:** &lt;1ms (인메모리, 직접 호출)

### 메모리

**호출당 오버헤드:** ~1KB (메시지 버퍼)

**제로 복사 최적화:** 대용량 데이터 (>1MB)는 가능한 경우 공유 메모리를 사용합니다.
