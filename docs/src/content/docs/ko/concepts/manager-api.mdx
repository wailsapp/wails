...
Wails v3 매니저 API는 집중된 매니저 구조체를 통해 애플리케이션 기능에 접근할 수 있는 조직적이고 탐색 가능한 방법을 제공합니다. 이 새로운 API 구조는 기존 App API와의 완전한 하위 호환성을 유지하면서 관련 메서드들을 함께 그룹화합니다.

## 개요

매니저 API는 애플리케이션 기능을 열한 가지 집중된 영역으로 조직화합니다:
...
- **향상된 코드 조직화** - 관련 메서드들이 함께 그룹화됨
- **향상된 유지보수성** - 매니저 간 관심사 분리
- **미래 확장성** - 특정 영역에 새 기능을 추가하기 쉬움
