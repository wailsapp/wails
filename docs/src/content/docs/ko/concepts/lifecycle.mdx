...
서비스는 Wails v3에서 수명 주기를 관리하는 주요 방법입니다. 인터페이스를 통해 시작 및 종료 후크를 제공합니다. 서비스에 대한 전체 문서는 [서비스 가이드](/ko/features/bindings/services)를 참조하세요.
...
시작 로직, 의존성 주입, 또는 상태 유지 리소스가 포함된 더 복잡한 수명 주기 관리를 위해서는 [서비스](#services-lifecycle)를 대신 사용하세요.

### ShouldQuit

`ShouldQuit` 콜백은 사용자가 마지막 창을 닫거나, Cmd+Q (macOS) / Alt+F4 (Windows)를 누르거나, `app.Quit()`를 프로그래밍 방식으로 호출하는 등 종료가 요청될 때마다 호출됩니다.

**반환 값:**
- `true`를 반환하여 종료를 진행하도록 허용함 (애플리케이션이 종료됨)
- `false`를 반환하여 종료를 취소함 (애플리케이션이 계속 실행됨)
...
app := application.New(application.Options{
    ShouldQuit: func() bool {
        if !hasUnsavedChanges() {
            return true  // 저장되지 않은 변경 사항이 없음, 종료 허용
        }
...
- macOS에서 사용자가 Cmd+Q를 누름
- Windows에서 사용자가 마지막 창에 포커스된 상태에서 Alt+F4를 누름
- 코드에서 `app.Quit()`를 호출함
...
app := application.New(application.Options{
    OnShutdown: func() {
        ...
        // 연결 닫기----------|--------------------------------|
| macOS | 앱이 실행 상태 유지(메뉴 막대 유지) |
| Windows | 앱 종료 |
| Linux | 앱 종료 |

macOS는 창이 없어도 메뉴 막대에 애플리케이션이 활성 상태로 남아 있는 네이티브 플랫폼 관례를 따릅니다. Windows와 Linux는 기본적으로 종료됩니다.
