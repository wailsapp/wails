...
2.  ## 프로젝트 구조 탐색
    ...
    :::note
    Wails v3는 기본 빌드 시스템으로 [Task](https://taskfile.dev/)를 사용하지만, `make` 또는 다른 대안 빌드 시스템을 사용하는 것을 막는 것은 아무것도 없습니다.
    :::
...
4.  ## 개발 모드
    ...
    애플리케이션을 개발 모드에서도 실행할 수 있습니다. 이 모드를 사용하면 프론트엔드 코드를 변경하고 전체 애플리케이션을 다시 빌드하지 않고도 실행 중인 애플리케이션에 변경 사항을 반영할 수 있습니다.
...
5.  ## 애플리케이션 패키징
    ...
    패키징 옵션과 구성에 대한 자세한 정보는 [패키징 가이드](/guides/packaging)를 참조하세요.
...
이제 첫 Wails 애플리케이션을 생성하고, 개발하고, 패키징하는 방법을 배웠습니다.
이것은 Wails v3로 달성할 수 있는 것의 시작일 뿐입니다.

## 다음 단계

Wails가 처음이라면, Wails의 다양한 기능을 실습적으로 안내하는 [튜토리얼](/tutorials)을 읽어보시길 권장합니다. 첫 번째 튜토리얼은 [서비스 생성하기](/ko/tutorials/01-creating-a-service)입니다.

더 고급 사용자의 경우, Wails 사용 방법에 대한 더 자세한 정보는 [가이드](/guides)를 확인하세요.