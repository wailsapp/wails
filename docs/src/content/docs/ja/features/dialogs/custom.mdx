...
通常のWailsウィンドウを使用して、ダイアログのような動作をする**カスタムダイアログウィンドウ**を作成します。カスタムフォーム、複雑な入力検証、ブランドに合わせた外観、リッチコンテンツ（画像、動画）を実装しながら、親しみやすいダイアログのパターンを維持します。
...
**ご質問がありますか？** [Discord](https://discord.gg/JDdSxwjhGf) で質問するか、[カスタムダイアログの例](https://github.com/wailsapp/wails/tree/master/v3/examples/dialogs) をご確認ください。