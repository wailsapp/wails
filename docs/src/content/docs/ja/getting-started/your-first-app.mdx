---
title: 初めてのアプリケーション
description: ステップバイステップで最初の Wails デスクトップアプリケーションを作成する方法
sidebar:
  order: 20
---
...
2.  ## プロジェクト構造の探索
    ...
    - build/           ビルドプロセスで使用されるファイル
        ...
        - darwin/      macOS 固有のビルドファイル
            ...
            - Info.plist    本番用設定
            - Taskfile.yml  macOS 用ビルドタスク
            - icons.icns    macOS アプリケーションのアイコン
        - linux/       Linux 固有のビルドファイル
            - Taskfile.yml  Linux 用ビルドタスク
        ...
        - windows/     Windows 固有のビルドファイル
            - Taskfile.yml        Windows 用ビルドタスク
            - icon.ico           Windows アプリケーションのアイコン
    ...
    これらのファイルを見て回り、構造に慣れてください。

    :::note
    Wails v3 はデフォルトで [Task](https://taskfile.dev/) をビルドシステムとして使用していますが、`make` や他の代替ビルドシステムを使用することを妨げるものはありません。
    :::
...
3.  ## アプリケーションのビルド
    ...
    ビルドが完了したら、通常のアプリケーションと同様に実行できます：
    ...
    アプリケーションの起点となるシンプルな UI が表示されます。デバッグバージョンであるため、コンソールウィンドウにもログが表示されます。これはデバッグ目的で役立ちます。

4.  ## 開発モード

    アプリケーションを開発モードで実行することもできます。このモードでは、フロントエンドコードを変更しても、アプリケーション全体を再ビルドすることなく、実行中のアプリケーションに変更を反映させることができます。

    1. 新しいターミナルウィンドウを開きます。
    2. `wails3 dev` を実行します。アプリケーションがデバッグモードでコンパイルされ、実行されます。
    3. お好みのエディターで `frontend/index.html` を開きます。
    4. コードを編集し、`Please enter your name below` を `Please enter your name below!!!` に変更します。
    5. ファイルを保存します。

    この変更はアプリケーションにすぐに反映されます。
...
5.  ## アプリケーションのパッケージング
    ...
    <TabItem label="Mac" icon="apple">
            ...
            <TabItem label="Linux" icon="linux">
              ...
              Wails は Linux 配布用の複数のパッケージ形式をサポートしています：
        ...
        パッケージングのオプションと設定に関する詳細については、[パッケージングガイド](/guides/packaging) を参照してください。
...
プロジェクト作成時に `-git` フラグを使用することで、初期化の全手順を自動化できます:
```bash
wails3 init -n myfirstapp -git github.com/username/myfirstapp
```
これは以下の様々な Git URL 形式に対応しています:
...
Wails が初めての方は、Wails の様々な機能を実践的にガイドする [チュートリアル](/tutorials) を読むことをお勧めします。最初のチュートリアルは [サービスを作成する](/ja/tutorials/01-creating-a-service) です。
