...
デスクトップアプリケーションには、起動からシャットダウンまでのライフサイクルが存在します。Wails v3 では、このライフサイクルを効果的に管理するために **サービス (Services)**、**イベント (Events)**、および **フック (Hooks)** を提供しています。
...
- OS イベント（マウス、キーボード、ウィンドウイベント）が処理されます
- Go から JS へのメッセージが処理されます
- JS から Go への呼び出しが実行されます
- UI の更新が描画されます
...
3. サービスは登録の逆順でシャットダウンされます
4. ウィンドウが閉じられます
5. リソースが解放されます

## サービスのライフサイクル

サービスは、Wails v3 でライフサイクルを管理するための主要な方法です。サービスは、インターフェースを通じて起動およびシャットダウンフックを提供します。サービスに関する完全なドキュメントについては、[サービスのガイド](/ja/features/bindings/services) を参照してください。
...
これらは `application.Options` 内の便利なコールバックであり、完全なサービスを作成せずにアプリケーションのライフサイクルにフックすることを可能にします。単純なクリーンアップタスク、終了の確認、またはシャットダウンシーケンスの特定のポイントでコードを実行する必要がある場合に便利です。
...
`OnShutdown` コールバックは、アプリケーションが終了することが確定されたとき（`ShouldQuit` が設定されている場合は `true` を返した後）に呼び出されます。状態の保存、データベース接続の閉鎖、リソースの解放などのクリーンアップタスクに使用してください。
...
app := application.New(application.Options{
    OnShutdown: func() {
        ...
        // Close connections----------|--------------------------------|
| macOS | アプリケーションは実行中のまま（メニューバーが残る） |
...
**すべてのプラットフォームで最後のウィンドウが閉じた後も実行中のままにする：**

これは、システムトレイアプリケーションや、バックグラウンドで実行し続ける必要があるアプリケーションに有用です。
