...
@note{type="tip" title="TL;DR - Experienced Developers"}
```bash
...
Or manually verify with `wails3 doctor`. [Skip to First App →](/quick-start/first-app/)
...
@note{type="tip" title="Performance Tip for Windows 11"}
...
@note{type="caution" title="Minimum distro versions"}
...
Applications playing audio or video also need the GStreamer codecs for their chosen formats. See [Play Local Audio and Video](/guides/linux-media/) for local media setup and playback troubleshooting.
...
The setup wizard is new and primarily tested on Linux. If you encounter issues, [report them](https://github.com/wailsapp/wails/issues/4904) and use `wails3 doctor` instead.
...
@note{type="info" title="If `wails3` command not found"}
...
[macOS]
**Option 1: Official Installer** Download from [nodejs.org](https://nodejs.org/)
...
@note{type="tip" title="Alternative Package Managers"}
...
@note{type="caution" title="Development Version"}
