...
基準となる PNG ソースから Windows、macOS、Linux hicolor のアイコンを生成します。
...
Linux の PNG は再現可能な生成アセットです。固定サイズのファイルは、DEB、RPM、Arch のデフォルトのパッケージ設定によって対応する hicolor ディレクトリにインストールされます。
