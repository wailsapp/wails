...
アプリケーションは、機密データをマスクするサニタイザーを提供します。IPC ログはデフォルトで自動的にサニタイズされます。

### Sanitizer()

アプリケーションのサニタイザーインスタンスを返します。
...
アプリケーションオプションの `SanitizeOptions` でサニタイズを設定します。
...
詳しいドキュメントは[セキュリティガイド](/guides/security/)を参照してください。
