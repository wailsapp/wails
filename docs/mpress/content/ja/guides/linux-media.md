...
ファイルは 64 KiB のフレームで転送されます。Wails のデスクトップストリーム転送はポーリング応答を 1 MiB に制限するため、完全な応答をバッファリングする WebView2 でも、メディアファイル全体が1つの応答に蓄積されることはありません。ストリームキューにも独自の上限付きバックプレッシャーがあります。これらの転送制限は、プレーヤーヘルパーが完全な Blob を扱う動作を変えません。
