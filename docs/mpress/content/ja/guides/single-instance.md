...
- `ExitCode`: 最初のインスタンスに通知した後、2 番目のインスタンスが終了するときの終了コード
- `OnSecondInstanceExit`: 最初のインスタンスへの通知後、終了直前に 2 番目のインスタンスで実行されるコールバック。終了には `os.Exit` を使用するため、`main` の defer 関数は実行されません。`application.New` より前に開いたリソースの解放やバッファーのフラッシュはここで行ってください
