...
#### アプリケーションのライフサイクルイベント (Android, iOS, macOS)

これらのイベントは Android、iOS、macOS でのみ発行されます。Windows と Linux には、アプリケーション単位での同等のフォアグラウンド／バックグラウンド遷移はありません（これらのプラットフォームでマシンのスリープ／復帰を扱う場合は `common:SystemWillSleep`/`SystemDidWake` を使用してください）。

| イベント | 説明 | 使用する場面 |
| --- | --- | --- |
| `common:ApplicationResumed` | アプリケーションがアクティブになった（フォアグラウンドで操作可能）。macOS：アプリがアクティブ化された。モバイル：フォアグラウンドに戻った。 | サービスへの再接続、描画の再開 |
| `common:ApplicationPaused` | アプリケーションがフォーカスを失う、または非アクティブになろうとしている。macOS：アプリが非アクティブになった。モバイル：バックグラウンドへの移行中。 | 負荷の高い処理の一時停止、一時的な状態の保存 |
| `common:ApplicationBackgrounded` | アプリケーションがバックグラウンドに移行した。macOS：アプリが非表示になった。モバイル：完全にバックグラウンドになった。 | 状態の永続化、リソースの解放、ネットワーク処理の一時停止 |
| `common:ApplicationForegrounded` | アプリケーションがアクティブになる前にフォアグラウンドへ戻りつつある。macOS：アプリを再表示中。モバイル：バックグラウンドからの復帰中。 | 再開の準備、古いデータの更新 |
