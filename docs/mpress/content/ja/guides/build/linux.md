...
同梱クリップには、[ローカルの音声と動画を再生する](/guides/linux-media/)で説明する `Media.SetSource` と `Media.ClearSource` を使用してください。ガイドではメディアファイルシステムの登録、読み込みのキャンセル、Blob の解放、サイズ制限を説明します。転送は待ち受けソケットを開かずに Wails ストリームを使用し、各クリップは再生前に完全に読み込まれます。フロントエンドのデフォルト制限は 32 MiB です。

配布要件に、メディア形式に必要な GStreamer コーデックを含めてください。転送の成功は、対象システムがファイルをデコードできることを保証しません。サポートする Linux ディストリビューションで再生をテストしてください。

## トラブルシューティング

### ローカルの音声や動画を再生できない

GStreamer が **No URI handler implemented for "wails"** と報告する場合は、[ローカルメディア再生ガイド](/guides/linux-media/)に従ってください。読み込みに成功しても再生に失敗する場合は、コーデック要件とプレーヤーの `error` イベントを確認してください。
