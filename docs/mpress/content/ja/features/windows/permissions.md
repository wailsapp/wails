...
この機能は `Permissions` を設定しなくても動作します。マップを設定した場合、macOS は Web コンテンツからのカメラとマイクの要求に適用します。`PermissionDeny` はシステムの確認画面が出る前に要求を拒否し、`PermissionAllow` はユーザーがシステムレベルで既に許可した範囲でアクセスを許可し、`PermissionDefault`（デフォルト）はシステムの確認画面を表示します。位置情報と通知は、マップに関係なく引き続きシステムのプライバシーフレームワークを通じて処理されます。Go からシステムレベルの権限を要求または確認するには、[macOS プラットフォーム統合ガイド](/guides/macos-platform-integration/)で説明する `app.Permissions` を使用してください。
...
macOS では、`Permissions` マップが影響するのは Web コンテンツからのカメラとマイクの要求だけです。全体的なアクセス可否は引き続きシステムのプライバシーフレームワークが決定します。`Info.plist` に適切な使用目的の説明キー（`NSMicrophoneUsageDescription`、`NSCameraUsageDescription` など）が含まれ、ユーザーが「システム設定」→「プライバシーとセキュリティ」でアクセスを許可していることを確認してください。
