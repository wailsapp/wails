...
| `wails3 migrate` | Migriert ein V2-Projekt in ein separates V3-Ausgabeverzeichnis. Experimentell: `-d` für das V2-Projekt, `-o` für die Ausgabe, `-f` erlaubt ein nicht leeres Verzeichnis, `-q` unterdrückt Ausgaben und `-skipgomodtidy` überspringt die Abhängigkeitsbereinigung. Prüfen Sie `MIGRATION.md`; Anwendungslogik und übrige V2-API-Aufrufe müssen manuell portiert werden. |
