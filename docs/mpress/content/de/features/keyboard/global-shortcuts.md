...
Tastenkürzel mit Buchstaben berücksichtigen das Tastaturlayout. Die Carbon-API bindet Tastenkürzel an physische Tastenpositionen. Deshalb ermittelt Wails mit `UCKeyTranslate` die Taste, die den Buchstaben im aktiven Tastaturlayout erzeugt. `Cmd+A` verwendet somit bei AZERTY, QWERTZ, Dvorak und ähnlichen Layouts die mit **A** beschriftete Taste statt der QWERTY-Position. Ziffern, Satzzeichen und benannte Tasten (Pfeiltasten, Funktionstasten usw.) behalten ihre feste physische Position. Die Zuordnung erfolgt beim Registrieren des Tastenkürzels. Wenn Sie das Tastaturlayout während der Ausführung der Anwendung wechseln, bleiben bestehende Buchstabenkürzel an die ursprünglich zugewiesene physische Taste gebunden.
