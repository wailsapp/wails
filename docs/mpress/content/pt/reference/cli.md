...
| `wails3 migrate` | Migra um projeto V2 para um diretório V3 separado. Experimental: `-d` indica o projeto V2, `-o` a saída, `-f` permite diretório não vazio, `-q` silencia a saída e `-skipgomodtidy` ignora a limpeza de dependências. Revise `MIGRATION.md`; a lógica e as chamadas restantes à API V2 precisam de portabilidade manual. |
