---
title: "Integração com o navegador"
description: "Abra URLs e arquivos no navegador padrão do usuário"
slug: "features/browser/integration"
sourcePath: "features/browser/integration.md"
---

O Wails oferece uma integração simples com o navegador por meio da API BrowserManager, permitindo que seu aplicativo abra URLs e arquivos no navegador padrão do usuário. Isso é útil para abrir links externos, documentação ou arquivos que devem ser processados pelo navegador.

## Acessando o gerenciador do navegador

O gerenciador do navegador é acessado por meio da propriedade `Browser` da instância do seu aplicativo:
...
Abra URLs no navegador padrão do usuário:
...
Abra servidores de desenvolvimento locais ou recursos da rede local:
...
Abra diversos tipos de arquivo que os navegadores podem processar:
...
Facilite o acesso a recursos de ajuda:
...
Processe links externos presentes no conteúdo do seu aplicativo:
...
**API privada no macOS:** `OpenDevTools()` requer `private_mac_apis`; caso contrário, a chamada não realiza nenhuma operação. As compilações de produção também exigem `devtools`. Consulte a [matriz de compilação do Web Inspector](/guides/build/private-macos-apis/#web-inspector).

## Tratamento de erros

### Tratamento adequado de erros

Sempre trate possíveis erros ao abrir URLs ou arquivos:
...
- Usa o comando `open` para iniciar o navegador padrão
- Respeita a configuração do navegador padrão do usuário nas Preferências do Sistema
- Pode solicitar permissão se o aplicativo estiver em um sandbox
- Processa corretamente URLs `file://` de arquivos locais
...
- Processa corretamente os formatos de caminho do Windows
- Pode exibir avisos de segurança para URLs não confiáveis

[Linux]
No Linux:

- Primeiro tenta usar `xdg-open` e, se isso não funcionar, recorre a outros métodos
- O comportamento varia conforme o ambiente de desktop
- Respeita a variável de ambiente `BROWSER`, caso esteja definida
...
1. **Sempre trate os erros**: as operações do navegador podem falhar por diversos motivos:
  ```go
...
2. **Valide as URLs**: verifique se as URLs estão bem-formadas antes de abri-las:
  ```go
...
3. **Confirmação do usuário**: para links externos, considere solicitar a permissão do usuário:
  ```go
...
4. **Caminhos de arquivo seguros**: ao abrir arquivos, verifique se os caminhos são seguros:
  ```go
...
Veja um exemplo completo que demonstra vários padrões de integração com o navegador:
...
Considere fornecer mecanismos alternativos para quando as operações do navegador falharem, como copiar URLs para a área de transferência ou exibi-las em uma caixa de diálogo para abertura manual.

@end

@note{type="danger" title="Aviso"}
Sempre valide URLs e caminhos de arquivo antes de abri-los para evitar problemas de segurança. Tenha cuidado ao abrir URLs fornecidas pelo usuário sem validação.
