...
- **Per-platform managers** — `application.IOS` (in `//go:build ios` files) and `application.Android` (in `//go:build android` files). Use these for anything platform-specific. See the [iOS](/guides/mobile/ios/) and [Android](/guides/mobile/android/) references for the full per-platform surface.
...
@note{type="tip"}
The `MobileManager` interface is the contract behind `application.Mobile`. Because both platform managers must satisfy it, any method listed above is guaranteed to keep an identical signature on iOS and Android — if they ever drift, the platform build fails to compile.
