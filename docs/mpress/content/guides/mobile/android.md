...
@note{type="tip"}
With [Play App Signing](https://support.google.com/googleplay/android-developer/answer/9842756), the keystore you sign with locally is your **upload key**: Google verifies your upload with it, then re-signs the app with the app signing key it manages. Also note that Google Play requires a higher `versionCode` for every upload; bump it in `build/android/app/build.gradle`.
