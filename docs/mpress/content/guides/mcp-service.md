...
@note{type="caution" title="Experimental Feature"}
The built-in MCP server is experimental and the API may change in future releases.

@end

Wails v3 has a built-in [Model Context Protocol](https://modelcontextprotocol.io) (MCP) server that lets LLM agents — Claude Code, IDE assistants, or any MCP client — inspect, test, and drive a **running** Wails application.

For project lifecycle automation, use the separate `wails3 mcp` CLI server. It lets agents inspect and initialize projects, run diagnostics, start builds and development jobs, generate bindings, run named Taskfile tasks, and retrieve bounded job output. The CLI server is confined to the current directory by default and does not expose arbitrary shell execution. See the [CLI MCP documentation](/guides/cli/#mcp) for transport, authentication, and tool details.
...
@note{type="caution"}
The MCP server gives full programmatic control over your application. Anyone who can access its tools can read the DOM, evaluate JavaScript, click buttons, and call Go methods.

@end

- The server binds to `127.0.0.1` by default. Browser origins must be HTTP(S) loopback origins; opaque (`null`), malformed, and foreign origins are rejected.
- Headerless native MCP clients remain supported. Without `WAILS_MCP_TOKEN`, local processes and permitted local browser origins are trusted; origin checks are not authentication. Set a high-entropy token to require bearer authentication for all `/mcp` calls. Configure the same token in the client's `Authorization: Bearer <token>` header. Preflight requests do not require the token.
- The `/eval-result` callback uses unpredictable per-evaluation IDs rather than the client bearer token, so webview result delivery remains compatible.
...
- If you need to expose the server on a non-loopback interface (e.g. LAN testing), set `WAILS_MCP_HOST=0.0.0.0` and a high-entropy `WAILS_MCP_TOKEN`; startup fails without a token. Use an encrypted tunnel or TLS-terminating proxy on untrusted networks because the built-in listener uses HTTP.

## Example application

A complete playground application demonstrating all tools is available at [`v3/examples/mcp`](https://github.com/wailsapp/wails/tree/releases/v3-beta/v3/examples/mcp). It includes:
