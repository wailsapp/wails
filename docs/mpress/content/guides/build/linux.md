...
@note{type="caution" title="Legacy path"}
...
For bundled clips, use `Media.SetSource` and `Media.ClearSource` as described in
[Play Local Audio and Video](/guides/linux-media/). The guide covers registering
a media filesystem, cancelling loads, releasing blobs and setting size limits.
Transfers use Wails streams without opening a listening socket; each clip is
fully loaded before playback, with a 32 MiB default frontend limit.

Include the GStreamer codecs required by your media formats in your distribution
requirements. A successful transfer does not guarantee the target system can
decode the file. Test playback on the Linux distributions you support.

## Troubleshooting

### Local audio or video will not play

If GStreamer reports **No URI handler implemented for "wails"**, follow the
[local media playback guide](/guides/linux-media/). If the clip loads but playback
fails, check its codec requirements and the player's `error` event.
