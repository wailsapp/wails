...
- `ExitCode`: 첫 번째 인스턴스에 알린 후 두 번째 인스턴스가 종료할 때 사용하는 종료 코드
- `OnSecondInstanceExit`: 첫 번째 인스턴스에 알린 후 종료 직전에 두 번째 인스턴스에서 실행되는 콜백입니다. `os.Exit`로 종료하므로 `main`의 지연 함수(defer)는 실행되지 않습니다. `application.New` 전에 연 리소스를 해제하거나 버퍼를 비우는 작업을 여기서 수행하세요
