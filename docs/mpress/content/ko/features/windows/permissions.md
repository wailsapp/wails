...
`Permissions`를 구성하지 않아도 작동합니다. 맵을 설정하면 macOS는 웹 콘텐츠의 카메라 및 마이크 요청에 이를 적용합니다. `PermissionDeny`는 시스템 권한 요청 전에 요청을 거부하고, `PermissionAllow`는 사용자가 시스템 수준에서 이미 허용한 범위 안에서 권한을 부여하며, `PermissionDefault`(기본값)는 시스템 권한 요청을 표시합니다. 위치 정보와 알림은 맵 설정과 관계없이 계속 시스템 개인정보 보호 프레임워크를 거칩니다. Go에서 시스템 수준 권한을 요청하거나 확인하려면 [macOS 플랫폼 통합 가이드](/guides/macos-platform-integration/)에 설명된 `app.Permissions`를 사용하세요.
...
macOS에서 `Permissions` 맵은 웹 콘텐츠의 카메라 및 마이크 요청에만 영향을 줍니다. 전체 접근 권한은 여전히 시스템 개인정보 보호 프레임워크가 결정합니다. `Info.plist`에 올바른 사용 목적 설명 키(`NSMicrophoneUsageDescription`, `NSCameraUsageDescription` 등)가 포함되어 있고 사용자가 시스템 설정 → 개인정보 보호 및 보안에서 접근을 허용했는지 확인하세요.
