...
@note{type="tip" title="Prerequisites"}
- Go 1.25 or newer
...
@note{type="note" title="Version string format"}
...
@note{type="caution" title="Run it in a goroutine"}
...
@note{type="note" title="Asset naming"}
...
![Default updater window in the Update Ready state, showing the version pill, Markdown-rendered release notes, and the Restart & Apply primary button.](/assets/updater/default-window-ready.png)
...
### Wire `currentVersion` to the build
...
![A bring-your-own updater window with a pink-orange gradient background and a custom rounded card layout, demonstrating that the default UI is fully replaceable.](/assets/updater/byo-custom-window.png)

@note{type="caution" title="`AllowSimpleEventEmit` is required"}
