...
@note{type="info"}
Simplified import paths are only available when using frontend bundlers. If you prefer a vanilla frontend that does not employ a bundler, you will have to import either `index.js` or `qrservice.js` manually.
...
### Alternative Approach: HTTP Handler
...
@note{type="info"}
If you do not set the `Route` option explicitly, the HTTP handler won't be accessible from the frontend.
...
@note{type="info"}
If a Go method returns a non-nil error, the promise on the JS side will reject and await statements will throw an exception.
