...
In Wails v3 there are **no** package-level dialog constructors — every dialog is created through the `app.Dialog` manager. From a service, get the app via `application.Get()` (it returns the running `*application.App`):
...
The matching methods are `app.Dialog.Info() / Question() / Warning() / Error()` for message dialogs and `app.Dialog.OpenFile() / SaveFile()` (plus the `*WithOptions` variants) for file dialogs.
