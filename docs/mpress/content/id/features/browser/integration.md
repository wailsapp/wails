...
Wails menyediakan integrasi browser sederhana melalui API BrowserManager, sehingga aplikasi Anda dapat membuka URL dan file di browser web default pengguna. Fitur ini berguna untuk membuka tautan eksternal, dokumentasi, atau file yang sebaiknya ditangani oleh browser.

## Mengakses Browser Manager

Browser Manager diakses melalui properti `Browser` pada instans aplikasi Anda:
...
Buka server pengembangan lokal atau sumber daya jaringan lokal:
...
### Membuka Jenis File Lain

Buka berbagai jenis file yang dapat ditangani oleh browser:
...
Sediakan akses mudah ke sumber daya bantuan:
...
### Mengekspor dan Melihat Laporan
...
Buka sumber daya pengembangan selama proses pengembangan:
...
**API privat di macOS:** `OpenDevTools()` memerlukan `private_mac_apis`; jika tidak, pemanggilan tidak melakukan apa pun. Build produksi juga memerlukan `devtools`. Lihat [matriks build Web Inspector](/guides/build/private-macos-apis/#web-inspector).

## Penanganan Kesalahan

### Penanganan Kesalahan yang Baik

Selalu tangani potensi kesalahan saat membuka URL atau file:
...
- Menggunakan perintah `open` untuk menjalankan browser default
- Mengikuti pengaturan browser default pengguna di System Preferences
- Dapat meminta izin jika aplikasi berjalan dalam sandbox
- Menangani URL `file://` untuk file lokal dengan benar

[Windows]
Di Windows:

- Menggunakan Windows Shell API untuk membuka URL
- Mengikuti pengaturan browser default di Windows Settings
- Menangani format jalur Windows dengan benar
- Dapat menampilkan peringatan keamanan untuk URL yang tidak tepercaya

[Linux]
Di Linux:

- Mencoba menggunakan `xdg-open` terlebih dahulu, lalu beralih ke metode lain jika gagal
- Perilaku bervariasi menurut lingkungan desktop
- Mengikuti variabel lingkungan `BROWSER` jika ditetapkan
- Mungkin memerlukan paket tambahan pada instalasi minimal

@end

## Praktik Terbaik

1. **Selalu Tangani Kesalahan**: Operasi browser dapat gagal karena berbagai alasan:
  ```go
...
2. **Validasi URL**: Pastikan format URL valid sebelum membukanya:
  ```go
...
3. **Konfirmasi Pengguna**: Untuk tautan eksternal, pertimbangkan untuk meminta izin pengguna:
  ```go
...
4. **Jalur File yang Aman**: Saat membuka file, pastikan jalurnya aman:
  ```go
...
@note{type="tip" title="Tips Profesional"}
Pertimbangkan untuk menyediakan mekanisme alternatif ketika operasi browser gagal, seperti menyalin URL ke papan klip atau menampilkannya dalam dialog agar dapat dibuka secara manual.

@end

@note{type="danger" title="Peringatan"}
Selalu validasi URL dan jalur file sebelum membukanya untuk mencegah masalah keamanan. Berhati-hatilah saat membuka URL yang diberikan pengguna tanpa validasi.
