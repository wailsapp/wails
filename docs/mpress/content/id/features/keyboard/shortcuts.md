...
description: "Daftarkan pintasan keyboard global untuk mengakses fungsionalitas dengan cepat"
slug: "features/keyboard/shortcuts"
sourcePath: "features/keyboard/shortcuts.md"
---

Wails menyediakan sistem pengikatan tombol yang canggih untuk mendaftarkan pintasan keyboard global yang berfungsi di semua jendela aplikasi Anda. Dengan demikian, pengguna dapat mengakses fungsionalitas dengan cepat tanpa menelusuri menu.

## Mengakses Pengelola Pengikatan Tombol

Pengelola pengikatan tombol diakses melalui properti `KeyBindings` pada instans aplikasi Anda:
...
## Menambahkan Pengikatan Tombol

### Pengikatan Tombol Dasar

Daftarkan pintasan keyboard sederhana:
...
### Beberapa Pengikatan Tombol

Daftarkan beberapa pintasan untuk operasi umum:
...
## Akselerator Pengikatan Tombol

### Format Akselerator

Pengikatan tombol menggunakan format akselerator standar yang terdiri dari tombol pengubah dan tombol:
...
Tangani perbedaan antarplatform untuk pintasan umum:
...
## Mengelola Pengikatan Tombol

### Menghapus Pengikatan Tombol

Hapus pengikatan tombol saat tidak lagi diperlukan:
...
### Mendapatkan Semua Pengikatan Tombol

Ambil semua pengikatan tombol yang terdaftar:
...
### Pengikatan Tombol yang Peka Konteks

Buat pengikatan tombol peka terhadap konteks dengan memeriksa status aplikasi:
...
### Tindakan Khusus Jendela

Pengikatan tombol menerima jendela aktif sehingga memungkinkan perilaku khusus untuk setiap jendela:
...
### Pengelolaan Pengikatan Tombol Dinamis

Tambahkan dan hapus pengikatan tombol secara dinamis berdasarkan status aplikasi:
...
- Gunakan `Cmd`, bukan `Ctrl`, untuk pintasan standar
- `Cmd+Q` biasanya dikhususkan untuk keluar dari aplikasi
- `Cmd+H` menyembunyikan aplikasi
- `Cmd+M` meminimalkan jendela
- Pertimbangkan pintasan keyboard standar macOS
...
- Gunakan `Ctrl` untuk pintasan standar
- `Alt+F4` menutup aplikasi
- `F1` biasanya membuka bantuan
- Pertimbangkan konvensi keyboard Windows
...
- Umumnya mengikuti konvensi Windows dengan `Ctrl`
- Dapat berbeda menurut lingkungan desktop
- Pertimbangkan pintasan standar GNOME/KDE
- Beberapa lingkungan desktop mencadangkan pintasan tertentu
...
1. **Gunakan Pintasan Standar**: Ikuti konvensi platform untuk operasi umum:
  ```go
  // Cross-platform save
  if runtime.GOOS == "darwin" {
      app.KeyBinding.Add("Cmd+S", saveHandler)
...
2. **Berikan Umpan Balik Visual**: Beri tahu pengguna saat pintasan dipicu:
  ```go
  app.KeyBinding.Add("Ctrl+S", func(window application.Window) {
      saveDocument()
...
3. **Tangani Konflik**: Berhati-hatilah agar tidak menimpa pintasan sistem yang penting:
  ```go
...
4. **Dokumentasikan Pintasan**: Sediakan bantuan atau dokumentasi untuk pintasan yang tersedia:
  ```go
...
5. **Bersihkan**: Hapus pengikatan tombol sementara saat tidak lagi diperlukan:
  ```go
  func enterEditMode() {
...
Berikut contoh lengkap editor teks dengan pintasan keyboard:
...
@note{type="tip" title="Kiat Praktis"}
Uji pengikatan tombol Anda di semua platform target untuk memastikan semuanya berfungsi dengan benar dan tidak berkonflik dengan pintasan sistem.

@end

@note{type="danger" title="Peringatan"}
Berhati-hatilah agar tidak menimpa pintasan sistem yang sangat penting. Beberapa kombinasi tombol dicadangkan oleh sistem operasi dan tidak dapat ditangkap oleh aplikasi.
