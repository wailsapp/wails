...
Pintasan huruf mengikuti tata letak papan ketik. API Carbon mengikat pintasan ke posisi fisik tombol, sehingga Wails mencari tombol yang menghasilkan huruf tersebut pada tata letak aktif melalui `UCKeyTranslate`. Karena itu, `Cmd+A` menggunakan tombol berlabel **A** pada AZERTY, QWERTZ, Dvorak, dan tata letak serupa, bukan posisi QWERTY. Angka, tanda baca, dan tombol bernama (tombol panah, tombol fungsi, dan sebagainya) tetap menggunakan posisi fisik tetap. Pengikatan ditentukan saat pintasan didaftarkan; jika Anda mengganti tata letak saat aplikasi berjalan, pintasan huruf yang sudah ada tetap terikat ke tombol fisik semula.
