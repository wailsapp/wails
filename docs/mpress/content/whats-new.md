...
@note{type="tip" title="Multiple Windows"}
...
All the heavy lifting that the v2 build system did, such as icon generation and manifest creation, have been added as tool commands in the CLI. We have incorporated [Taskfile](https://taskfile.dev) into the CLI to orchestrate these calls to bring the same developer experience as v2. However, this approach brings the ultimate balance of flexibility and ease of use as you can now customise the build process to your needs.
...
@note{type="tip" title="Example of event handling"}
@details{title="Example"}
```go
package main
...
@note{type="tip" title="Example of wml"}
@details{title="Example"}
```html
<!doctype html>
<html lang="en">
...
There are more examples available in the [examples](https://github.com/wailsapp/wails/tree/master/v3/examples) directory. Check them out!
