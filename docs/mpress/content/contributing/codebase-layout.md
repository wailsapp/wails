...
Wails v3 lives in a **monorepo** that contains the framework runtime, CLI, examples, documentation, and build tool-chain. This page walks through the *directory structure* that matters to anyone digging into the internals.
...
Editing templates **does not** affect existing apps, only future `init`s. The public examples live under `v3/examples/`; they are not a substitute for the automated test suites described in the contributor documentation.
...
You now have a **mental map** of the repository. Use it with `ripgrep`, your IDE’s “Go to File/Symbol”, and the example apps to navigate deeper into any feature. Happy hacking!
