...
Wake is opt-in behind `WAILS_USE_WAKE=true` and is **not** the default runner. With the variable unset, `wails3 build / package / sign / task` behave exactly as before. Feature coverage and behaviour may change between releases.

@end

Wake is an **experimental alternative build runner** for `wails3`. It reads the same `Taskfile.yml` your project already has — the same task, dep, var, template, include and platform-namespace syntax — and runs it through a Wails-aware executor instead of the general-purpose [Task](https://taskfile.dev) runtime.
...
@note{type="tip" title="Enabling Wake is safe"}
If Wake encounters a Taskfile feature it doesn't implement, it hands the whole run off to the embedded Task runtime in-process — there's no external `task` binary to install. At worst you get exactly the behaviour you'd have had without the flag.
...
@note{type="note" title="Trust model"}
Override files are auto-discovered and applied with no prompt. This grants no new capability — a Taskfile already runs arbitrary shell commands, so an override can't do anything that editing `Taskfile.yml` couldn't. Committed `Taskfile.override.*` shows up in PR diffs; `Taskfile.local.*` is created on your own machine. A malformed override aborts the run rather than being silently skipped. Set `WAILS_NO_OVERRIDES=true` to skip override discovery entirely for deterministic CI builds.
...
Wake is an experiment, and your feedback decides where it goes. If you try it, we'd love to hear whether it was faster, clearer, and whether anything broke — the most useful reports say what you ran, what you expected and what actually happened. Tell us in the [Wake feedback discussion](https://github.com/wailsapp/wails/discussions/5679).
