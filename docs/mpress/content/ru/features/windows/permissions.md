...
В macOS необходимо согласие двух уровней. WKWebView обращается к приложению перед началом захвата, и карта `Permissions` отвечает на этот запрос. Ниже системная инфраструктура конфиденциальности TCC контролирует само устройство: системный запрос появляется при первом фактическом обращении приложения к камере или микрофону. Выбор сохраняется для каждого приложения в «Системные настройки» → «Конфиденциальность и безопасность».

Wails обрабатывает запросы **камеры и микрофона** в macOS 12 и новее. Для геолокации, уведомлений и чтения буфера обмена нет соответствующего механизма `WKUIDelegate`, поэтому они не подключены и остаются в ведении TCC. Как и в Linux, заданная для них политика не действует.

| Политика | Камера / Микрофон | Геолокация, Уведомления, Буфер обмена |
| --- | --- | --- |
| `PermissionDefault` | WebKit показывает собственный запрос | Только TCC |
| `PermissionAllow` | Запрос WebKit пропускается — **TCC продолжает действовать** | Только TCC |
| `PermissionDeny` | Отказ до обращения к устройству | Только TCC |

`PermissionAllow` разрешает запрос веб-представления, а не доступ к устройству. Первый захват по-прежнему вызывает запрос TCC. Если пользователь запретил приложение в системных настройках, запрет сохраняется: приложение не может само предоставить себе доступ. `PermissionAllow` устраняет только предварительный запрос WebKit.

До macOS 12 этот метод делегата отсутствует, поэтому карта игнорируется и все запросы возвращаются к запросу разрешения WebKit.
...
@note{type="caution" title="Запрещайте возможности, не объявленные в Info.plist"}

Если запрос достигает AVFoundation без соответствующего ключа описания назначения, он не просто завершается ошибкой: macOS завершает приложение.

`PermissionDefault` — нулевое значение. Поэтому одна запись `{PermissionMicrophone: PermissionAllow}` оставляет для камеры запрос WebKit. Если пользователь согласится, а приложение объявляет только `NSMicrophoneUsageDescription`, оно завершится. Явно задайте `PermissionDeny` для каждой возможности без описания назначения:
...
В **Linux** это явно разрешает доступ к обоим устройствам; остальные возможности остаются запрещёнными. В **Windows** это разрешает доступ к обоим устройствам; для любой другой возможности, не указанной вами, появится системный запрос. В **macOS** это разрешает оба устройства на уровне WebKit без запроса браузера. TCC по-прежнему запрашивает доступ к самим устройствам при первом использовании; оба ключа описания назначения должны присутствовать в `Info.plist`.
...
| Микрофон | ✅ | ✅ | ✅ (macOS 12+) |
| Камера | ✅ | ✅ | ✅ (macOS 12+) |
| Геолокация | ❌ пока нет | ✅ | ❌ пока нет |
| Уведомления | ❌ пока нет | ✅ | ❌ пока нет |
| Чтение из буфера обмена | ❌ пока нет | ✅ | ❌ пока нет |

При ✅ в macOS политика отвечает на запрос WebKit, а TCC дополнительно контролирует устройство. При ❌ возможность остаётся только в ведении TCC.
...
`Permissions` действует для камеры и микрофона в macOS 12 и новее. Геолокация, уведомления и чтение буфера обмена ещё не подключены и игнорируют политику. TCC продолжает контролировать устройство: `PermissionAllow` устраняет запрос WebKit, но не системный. Проверьте ключи `NSMicrophoneUsageDescription` и `NSCameraUsageDescription` в `Info.plist` и разрешение в «Системные настройки» → «Конфиденциальность и безопасность».

**Приложение macOS завершается, когда веб-контент запрашивает камеру или микрофон**

Если запрос достигает AVFoundation без соответствующего ключа описания назначения, он не просто завершается ошибкой: macOS завершает приложение. Добавьте соответствующий ключ или задайте для этой возможности `PermissionDeny`, чтобы запрос не достиг AVFoundation. `PermissionDefault` оставляет запрос WebKit, на который пользователь может согласиться.
