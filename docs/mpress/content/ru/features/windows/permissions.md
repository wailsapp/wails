...
Это работает без какой-либо настройки `Permissions`. Когда карта задана, macOS учитывает её для запросов доступа к камере и микрофону из веб-содержимого: `PermissionDeny` отклоняет запрос до появления системного запроса, `PermissionAllow` разрешает доступ в пределах того, что пользователь уже разрешил на системном уровне, а `PermissionDefault` (значение по умолчанию) позволяет показать системный запрос. Геолокация и уведомления независимо от карты по-прежнему проходят через системный механизм защиты конфиденциальности. Чтобы запросить или проверить системные разрешения из Go, используйте `app.Permissions`, описанный в [руководстве по интеграции с платформой macOS](/guides/macos-platform-integration/).
...
В macOS карта `Permissions` влияет только на запросы доступа к камере и микрофону из веб-содержимого; окончательное решение о доступе по-прежнему принимает системный механизм защиты конфиденциальности. Убедитесь, что в `Info.plist` указаны нужные ключи с описаниями использования (`NSMicrophoneUsageDescription`, `NSCameraUsageDescription` и т. д.) и что пользователь предоставил доступ в «Системные настройки → Конфиденциальность и безопасность».
