---
title: "Сервисы"
description: "Создавайте модульные, многократно используемые компоненты приложения с помощью сервисов"
...
**Сервисы** Wails позволяют структурировать логику приложения в виде модульных, самодостаточных компонентов. Сервисы учитывают жизненный цикл и предоставляют обработчики запуска и завершения работы, автоматически привязываются к фронтенду, поддерживают внедрение зависимостей и могут быть полностью протестированы изолированно.
...
**Готово!** Теперь `Greet` можно вызывать из JavaScript:
...
- Сервисы являются **одиночками** (один экземпляр)
- Методы могут возвращать `(value, error)`

### Предоставление безопасного интерфейса для фронтенда {#frontend-interface}

По умолчанию любой экспортируемый метод сервиса можно вызвать из фронтенда. Если у сервиса есть также методы только для бэкенда, зарегистрируйте его через именованный интерфейс с помощью `NewServiceAs`:
...
Wails генерирует привязки только для методов из `FrontendAuth`, а среда выполнения применяет тот же список разрешений к низкоуровневым вызовам как по имени, так и по идентификатору метода. Экспортируемые методы, например `Client`, `Pause` или `Resume`, остаются доступны в Go, но не регистрируются в мосте фронтенда.

Эта граница запрещает всё, что не разрешено явно: добавление экспортируемого метода в `AuthService` не открывает к нему доступ, пока он не добавлен и в `FrontendAuth`. Компилятор проверяет, что конкретный сервис реализует интерфейс.

@note{type="caution"}
Директивы генератора, такие как `//wails:ignore`, управляют генерируемым JavaScript и TypeScript, но не запрещают низкоуровневый вызов среды выполнения. Используйте проекцию интерфейса, если видимость методов задаёт границу безопасности или доверия.
@end

Требования и компромиссы проекции:

- Явно укажите тип именованного интерфейса: `NewServiceAs[FrontendAuth](authService)`.
- Передавайте указатель на конкретный сервис напрямую, чтобы генератор привязок мог определить его пакет и тип. Генерация привязок сообщит об ошибке, если сначала сохранить его в переменной интерфейсного типа или передать через обобщённую обёртку.
- Сгенерированный модуль и идентификаторы методов сохраняют идентичность конкретного сервиса, привычную структуру привязок Wails и поддержку обфусцированных сборок.
- Обработчики жизненного цикла, HTTP-маршрутизация, именование сервиса и доступ из бэкенда по-прежнему используют конкретный экземпляр; фильтруется только регистрация методов фронтенда.
- Интерфейс является дополнительным объявлением API, но заменяет фасадные структуры для перенаправления вызовов и упрощает проверку и создание тестовых имитаций открытого API.

Используйте `NewServiceAsWithOptions[FrontendAuth](authService, options)`, если проецируемому сервису также нужны [параметры сервиса](#--4).
...
**Важно:** сервисы совместно используются всеми окнами. Для обеспечения потокобезопасности всегда используйте мьютексы.
...
- Верните ошибку, чтобы **предотвратить запуск приложения**
- Используйте `ctx` для отмены
...
- Сервисы завершают работу **в обратном порядке**
- Контекст приложения уже отменён
- Верните ошибку, чтобы **записать предупреждение в журнал** (это не препятствует завершению работы)
...
## Шаблоны проектирования сервисов
...
- **Единственная ответственность** — один сервис, одна задача
...
- **Записывайте важные события в журнал** — используйте структурированное журналирование
- **Тестируйте изолированно** — используйте имитации зависимостей

### ❌ Не делайте так

- **Не используйте глобальное состояние** — передавайте зависимости
- **Не блокируйте запуск** — метод ServiceStartup должен выполняться быстро
- **Не пренебрегайте завершением работы** — всегда освобождайте ресурсы
- **Не создавайте циклические зависимости** — тщательно продумывайте архитектуру
- **Не делайте внутренние методы общедоступными** — оставляйте их закрытыми
- **Не забывайте о потокобезопасности** — сервисы используются совместно
...
- [Привязка методов](/features/bindings/methods/) — узнайте, как привязывать методы Go к JavaScript
- [Модели](/features/bindings/models/) — привязывайте сложные структуры данных
- [События](/features/events/system/) — используйте события для обмена данными по модели публикации и подписки
- [Рекомендации](/features/bindings/best-practices/) — шаблоны проектирования сервисов и рекомендации

---

**Есть вопросы?** Задайте их в [Discord](https://discord.gg/JDdSxwjhGf) или ознакомьтесь с [примерами сервисов](https://github.com/wailsapp/wails/tree/master/v3/examples).
