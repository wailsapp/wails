...
Для встроенных клипов используйте `Media.SetSource` и `Media.ClearSource`, как описано в руководстве [Воспроизведение локального аудио и видео](/guides/linux-media/). Оно охватывает регистрацию файловой системы медиа, отмену загрузок, освобождение blob и настройку ограничений размера. Передача использует потоки Wails без прослушивающего сокета; каждый клип полностью загружается перед воспроизведением, с ограничением фронтенда по умолчанию 32 MiB.

Включите кодеки GStreamer для ваших форматов медиа в требования к распространению. Успешная передача не гарантирует, что целевая система сможет декодировать файл. Проверяйте воспроизведение в поддерживаемых дистрибутивах Linux.

## Устранение неполадок

### Локальное аудио или видео не воспроизводится

Если GStreamer сообщает **No URI handler implemented for "wails"**, следуйте [руководству по воспроизведению локальных медиа](/guides/linux-media/). Если клип загружается, но не воспроизводится, проверьте требования к кодекам и событие `error` проигрывателя.
