...
#### События жизненного цикла приложения (Android, iOS, macOS)

Эти события отправляются только на Android, iOS и macOS. В Windows и Linux нет аналогичных переходов приложения между передним и задним планом (используйте `common:SystemWillSleep`/`SystemDidWake` для сна и пробуждения компьютера на этих платформах).

| Событие | Описание | Когда использовать |
| --- | --- | --- |
| `common:ApplicationResumed` | Приложение стало активным (на переднем плане и доступно для взаимодействия). В macOS: приложение активировано. На мобильных устройствах: возврат на передний план. | Восстановить соединения с сервисами, возобновить отрисовку |
| `common:ApplicationPaused` | Приложение теряет фокус или становится неактивным. В macOS: приложение перестало быть активным. На мобильных устройствах: переход в фоновый режим. | Приостановить затратные операции, сохранить временное состояние |
| `common:ApplicationBackgrounded` | Приложение перешло в фоновый режим. В macOS: приложение скрыто. На мобильных устройствах: полностью в фоновом режиме. | Сохранить состояние, освободить ресурсы, приостановить сетевую активность |
| `common:ApplicationForegrounded` | Приложение возвращается на передний план, ещё не став активным. В macOS: приложение снова показывается. На мобильных устройствах: возврат из фонового режима. | Подготовить возобновление работы, обновить устаревшие данные |
