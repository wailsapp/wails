...
- `ExitCode`: Код завершения второй копии после уведомления первой
- `OnSecondInstanceExit`: Обратный вызов, выполняемый во второй копии после уведомления первой, непосредственно перед завершением. Используется `os.Exit`, поэтому отложенные функции (defer) в `main` не выполняются. Освободите здесь ресурсы или сбросьте буферы, открытые до `application.New` в этом обратном вызове
