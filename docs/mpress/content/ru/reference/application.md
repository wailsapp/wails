...
Приложение предоставляет механизм очистки для скрытия конфиденциальных данных. Журналы IPC автоматически очищаются по умолчанию.

### Sanitizer()

Возвращает экземпляр механизма очистки приложения.
...
Настройте очистку через `SanitizeOptions` в параметрах приложения:
...
Полная документация приведена в [руководстве по безопасности](/guides/security/).
