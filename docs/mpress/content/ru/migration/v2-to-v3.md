...
- `main.go` преобразуется для использования `application.New()` и `app.Window.NewWithOptions()` с сохранением вашего кода и комментариев. Сопоставляются также платформенные параметры окон.
...
- `wails.json` заменяется системой сборки Taskfile и `build/config.yml` с метаданными продукта, ассоциациями файлов и протоколами из v2.
...
- Фронтенд копируется, а `@wailsio/runtime` добавляется в зависимости. Созданный v2 каталог `wailsjs/` не переносится, поскольку несовместим с v3.
