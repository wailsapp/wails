...
@note{type="warning" title="Экспериментальная функция"}
Команда `wails3 migrate` входит в CLI V3, но остаётся экспериментальной. Она поддерживает распространённые структуры проектов, однако результаты могут различаться: проверяйте созданный код и тщательно тестируйте приложение. При затруднениях [создайте issue](https://github.com/wailsapp/wails/issues) с подробностями. Pull request приветствуются.
@end

CLI может выполнить большую часть действий этого руководства:

```bash
wails3 migrate -d ./myv2project -o ./myv3project
```

Команда переносит однозначно сопоставимые части и документирует остальные. Она не переписывает логику приложения и не создаёт слои совместимости: вызовы API v2 остаются неизменными, а их расположение и конкретные замены v3 перечисляются в `MIGRATION.md`.

Автоматически переносятся:

- `main.go` преобразуется для использования `application.New()` и `app.Window.NewWithOptions()` с сохранением вашего кода и комментариев. Параметры, включая платформенные параметры окон, сопоставляются с их аналогами v3.
- Структуры из `Bind` становятся сервисами v3; `OnStartup`/`OnDomReady`/`OnShutdown`/`OnBeforeClose` подключаются к аналогам v3: событиям приложения, `OnShutdown` и `ShouldQuit`.
- `wails.json` заменяется файлами проекта v3: системой сборки Taskfile и `build/config.yml` с метаданными продукта, ассоциациями файлов и протоколами из v2.
- `go.mod` заменяет `wails/v2` на `wails/v3` и повышает старые директивы Go до `go 1.25`, минимальной версии для v3. Более новые версии сохраняются.
- Фронтенд копируется, а `@wailsio/runtime` добавляется в зависимости. Созданный каталог `wailsjs/` не переносится, поскольку содержит результаты сборки v2, несовместимые с v3.

В `MIGRATION.md` документируются:

- Все вызовы пакета `runtime` v2 с файлом, строкой и заменой v3; например, `runtime.EventsEmit(ctx, ...)` заменяется на `app.Event.Emit(...)`. Проект намеренно не компилируется до их переноса; компилятор указывает соответствующие места.
- Импорты фронтенда из `wailsjs/runtime` и `wailsjs/go/...`, эквиваленты в `@wailsio/runtime` и порядок генерации привязок через `wails3 generate bindings`.
- Параметры, требующие решения человека: меню, собственные средства журналирования, `EnumBind` и другие, с инструкциями.

Далее изменения рассматриваются подробно; используйте руководство вместе с созданным списком.
...
Команды разработки и сборки V3 задаются в корневом `Taskfile.yml`. Созданный проект хранит там команды фронтенда, а `build/config.yml` использует для настроек приложения и упаковки. Не создавайте V3-файл `wails.json`, копируя файл V2.
...
### Сообщения о проблемах миграции

Сбои инструмента миграции — это ошибки. Перед созданием issue сократите пример до минимального воспроизводимого проекта V2 и укажите:

- версию Wails V2 и версию или commit CLI V3;
- ОС и архитектуру;
- точную команду `wails3 migrate`;
- вывод команды и созданный `MIGRATION.md`;
- вывод `wails3 doctor`;
- очищенный от закрытых данных проект или минимальный пример, если код приватный.

Используйте шаблон ошибки в [трекере Wails](https://github.com/wailsapp/wails/issues). Не прикладывайте секреты, данные для подписи или приватный код.

Для изменения поведения миграции или API V3 предложите [Wails Enhancement Proposal](https://github.com/wailsapp/wails/tree/master/v3/wep) через pull request. Продуктовые предложения не следует оформлять как обычные запросы функций.
