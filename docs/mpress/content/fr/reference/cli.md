...
| `wails3 migrate` | Migre un projet V2 dans un dossier V3 distinct. Expérimental : `-d` indique le projet V2, `-o` le dossier de sortie, `-f` autorise un dossier non vide, `-q` active le mode silencieux et `-skipgomodtidy` omet le nettoyage des dépendances. Vérifiez `MIGRATION.md` ; la logique et les appels d’API V2 restants nécessitent un portage manuel. |
