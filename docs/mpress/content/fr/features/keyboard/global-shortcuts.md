...
Les raccourcis utilisant des lettres tiennent compte de la disposition du clavier. L’API Carbon associe les raccourcis à des positions physiques ; Wails recherche donc, avec `UCKeyTranslate`, la touche qui produit la lettre dans la disposition active. Ainsi, `Cmd+A` correspond à la touche marquée **A** sur les claviers AZERTY, QWERTZ, Dvorak et similaires, plutôt qu’à la position QWERTY. Les chiffres, les signes de ponctuation et les touches nommées (flèches, touches de fonction, etc.) conservent leur position physique fixe. L’association est déterminée lors de l’enregistrement du raccourci ; si vous changez de disposition pendant l’exécution de l’application, les raccourcis utilisant des lettres restent associés à leur touche physique d’origine.
