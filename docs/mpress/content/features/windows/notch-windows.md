...
@note{type="caution" title="Webview transparency uses a private API"}
`NewNotchWindow` requires `-tags private_mac_apis` for transparent web content to reveal the native notch shape. Without the tag, the panel, placement, and animations still work, but the webview stays opaque. See [Private macOS APIs](/guides/build/private-macos-apis/#webview-transparency-and-background).
...
<figure>
  <img
    ...
    alt="A Wails notch notification sliding down from the MacBook camera housing, displaying live system metrics, and hiding back beneath the notch"
    loading="lazy"
    decoding="async"
    style="width: 100%; border-radius: 0.75rem"
  />
  <figcaption>
    An animated notch notification using a persistent webview with native show
...
@note{type="note"}
Notch windows require macOS. On a Mac without a camera housing, Wails places the window at the top centre below the menu bar. On unsupported platforms, `NewNotchWindow` returns an inert handle whose lifecycle methods are safe no-ops and whose `Visibility` is always false.
...
@note{type="note"}
Persistent collapsed surfaces that reopen on hover are intentionally separate from notification hiding and are tracked in [#6009](https://github.com/wailsapp/wails/issues/6009).

@end

See the [notch-notification example](https://github.com/wailsapp/wails/tree/master/v3/examples/notch-notification) for a compact system monitor whose live JavaScript state survives repeated notification show and hide cycles.
