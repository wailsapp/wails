...
On **Linux** this explicitly allows both devices; other capabilities remain denied. On **Windows** this allows both; any other capability you don't list will show a native prompt. On **macOS** this allows both at the WebKit layer, so no browser prompt appears; TCC still asks for the devices themselves on first use, and both usage description keys must be present in `Info.plist`.
