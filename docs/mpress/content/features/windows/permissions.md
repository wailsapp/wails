...
This works without any `Permissions` configuration. When the map is set, macOS honours it for camera and microphone requests from web content: `PermissionDeny` refuses the request before the system prompt, `PermissionAllow` grants it within whatever the user has already allowed at the system level, and `PermissionDefault` (the default) lets the system prompt appear. Geolocation and notifications still go through the system privacy framework regardless of the map. To request or inspect system-level permissions from Go, use `app.Permissions`, described in the [macOS Platform Integration guide](/guides/macos-platform-integration/).
...
On **Linux** this explicitly allows both devices; other capabilities remain denied. On **Windows** this allows both; any other capability you don't list will show a native prompt. On **macOS** this has no effect; TCC handles everything.
...
On macOS the `Permissions` map only affects camera and microphone requests from web content; the system privacy framework still decides overall access. Ensure your `Info.plist` includes the correct usage description keys (`NSMicrophoneUsageDescription`, `NSCameraUsageDescription`, etc.) and that the user has granted access in System Settings → Privacy & Security.
