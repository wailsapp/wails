...
Removes the registration created under a specific `AutostartOptions.Identifier`, regardless of where the registered command points — use it to clean up a registration left behind by a moved or renamed executable. An empty `Identifier` keeps the discovery-based behaviour of `Disable`. Idempotent: removing an absent registration returns nil.
