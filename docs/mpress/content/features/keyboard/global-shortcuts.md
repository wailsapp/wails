...
@note{type="info" title="Global shortcuts vs key bindings"}
...
@note{type="tip" title="Touching the UI from a callback"}
...
Letter shortcuts are layout aware. The Carbon API binds to physical key positions, so Wails translates a letter to the key that produces it on your active keyboard layout (via `UCKeyTranslate`). `Cmd+A` therefore lands on the key labelled **A** on AZERTY, QWERTZ, Dvorak and similar layouts, rather than the QWERTY position. Digits, punctuation and named keys (arrows, function keys, and so on) use their fixed physical position. The binding is resolved when the shortcut is registered; if you switch keyboard layout while the application is running, existing letter shortcuts keep the physical key they were originally bound to.

@note{type="caution" title="Hide shortcuts and `ApplicationShouldTerminateAfterLastWindowClosed`"}
...
@note{type="danger" title="Avoid critical system shortcuts"}
