...
/*
#cgo freebsd pkg-config: gtk+-3.0
...
func NewWindow(appoptions *options.App, debug bool, devtoolsEnabled bool) *Window {
	...
	if appoptions.FreeBSD != nil {
		webviewGpuPolicy = int(appoptions.FreeBSD.WebviewGpuPolicy)
	} else {
		// workaround for https://github.com/wailsapp/wails/issues/2977
		webviewGpuPolicy = int(freebsd.WebviewGpuPolicyNever)
	}
	...
	if appoptions.FreeBSD != nil {
		if appoptions.FreeBSD.Icon != nil {
			result.SetWindowIcon(appoptions.FreeBSD.Icon)
		}
		if appoptions.FreeBSD.WindowIsTranslucent {
...
func (w *Window) SetBackgroundColour(r uint8, g uint8, b uint8, a uint8) {
	...
	if w.appoptions.FreeBSD != nil && w.appoptions.FreeBSD.WindowIsTranslucent {
