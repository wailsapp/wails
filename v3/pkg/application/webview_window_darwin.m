...
// delegate), reusing nsWindow's existing WebviewWindowDelegate so it
// participates in the same windowId-keyed asset/IPC routing as the window's
// main webview -- for any auxiliary webview that isn't the window's primary
// one (a split pane, a titlebar accessory). Used by webview_window_split_darwin.m
// and webview_window_titlebar_accessory_darwin.m.
//
// A zero-size starting frame silently suspends WKWebView's loading -- it
// never even reaches the scheme handler (confirmed empirically): callers
// must pass a non-degenerate width/height even though the eventual layout
// (auto layout constraints, a split view, etc.) will resize it.
WKWebView* buildAuxiliaryWebview(void* nsWindowPtr, double width, double height) {
    ...
    WKWebView* webView = [[WKWebView alloc] initWithFrame:NSMakeRect(0, 0, width, height) configuration:config]; // +1
