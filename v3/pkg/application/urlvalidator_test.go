...
func TestValidateURL(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			url:       "https://en.wikipedia.org/wiki/Tenth_Avenue_(Manhattan)",
			shouldErr: false,
			expected:  "https://en.wikipedia.org/wiki/Tenth_Avenue_(Manhattan)",
		},
		{
			name:      "URL with curly braces",
			url:       "https://example.com/{test}",
			shouldErr: false,
			expected:  "https://example.com/%7Btest%7D",
		},
		{
