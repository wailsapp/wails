		...
		func(next http.Handler) http.Handler {
			return http.HandlerFunc(func(rw http.ResponseWriter, req *http.Request) {
				...
				case "/wails/custom.js":
					// custom.js is only served in server mode; return 404 here so that
					// dev servers (e.g. Vite) don't fall back to serving index.html,
					// which would cause an "Unexpected token '<'" JavaScript error.
