...
func MenuBarWndProc(hwnd HWND, msg uint32, wParam WPARAM, lParam LPARAM, theme *MenuBarTheme) (bool, LRESULT) {
	...
	case WM_UAHMEASUREMENUITEM:
		...
		return true, result
	case WM_NCPAINT:
		// Let Windows do its default painting first
		result := DefWindowProc(hwnd, msg, wParam, lParam)

		// Then paint our custom menubar once on top
...
func paintDarkMenuBar(hwnd HWND, theme *MenuBarTheme) {
	...
	// Debug: Log painting coordinates
	// fmt.Printf("Painting menubar: Top=%d, Bottom=%d\n", menuBarInfo.Bar.Top, menuBarInfo.Bar.Bottom)
