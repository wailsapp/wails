  ...
  <body>
    <div>
      <h2 style="text-align: center;">This is a Window with a Tabbed-effect backdrop</h2>
		...
		// Custom Menubar Theme that respects system theme
		if runtime.GOOS == "windows" {
			myMenu.Add("New WebviewWindow (Custom Menubar - Follows System Theme)").
				OnClick(func(ctx *application.Context) {
					app.NewWebviewWindowWithOptions(application.WebviewWindowOptions{
						...
						Windows: application.WindowsWindow{
							Menu:  menu,
							Theme: application.SystemDefault, // Respect system theme
							CustomTheme: application.ThemeSettings{
								// Dark mode theme - Colorful example
								DarkModeMenuBar: &application.MenuBarTheme{
									Default: &application.TextTheme{
										Background: lo.ToPtr(uint32(0x00400080)), // Dark purple background
										Text:       lo.ToPtr(uint32(0x00FFFF00)), // Yellow text
									},
									Hover: &application.TextTheme{
										Background: lo.ToPtr(uint32(0x00600090)), // Lighter purple on hover
										Text:       lo.ToPtr(uint32(0x00FFFFFF)), // White text on hover
									},
									Selected: &application.TextTheme{
										Background: lo.ToPtr(uint32(0x008000A0)), // Even lighter purple when selected
										Text:       lo.ToPtr(uint32(0x00FFFFFF)), // White text when selected
									},
								},
								// Light mode theme
								LightModeMenuBar: &application.MenuBarTheme{
									Default: &application.TextTheme{
										Background: lo.ToPtr(uint32(0x00F0F0F0)), // Light gray background
										Text:       lo.ToPtr(uint32(0x00202020)), // Dark gray text
									},
									Hover: &application.TextTheme{
										Background: lo.ToPtr(uint32(0x00E0E0E0)), // Slightly darker on hover
										Text:       lo.ToPtr(uint32(0x00000000)), // Black text on hover
									},
									Selected: &application.TextTheme{
										Background: lo.ToPtr(uint32(0x00D0D0D0)), // Even darker when selected
										Text:       lo.ToPtr(uint32(0x00000000)), // Black text when selected
									},
								},
							},
						},
...
<html>
  ...
  <body>
    <h1>System Theme Aware</h1>
    <p>This window automatically uses the appropriate custom menubar theme based on your system settings!</p>
    <p>Dark mode: Dark purple menubar with yellow text<br>Light mode: Light gray menubar</p>
    <div style="background-color: rgba(255,200,0,0.2); padding: 15px; margin: 20px auto; max-width: 600px; border-radius: 8px; border: 2px solid rgba(255,200,0,0.5);">
      <p><strong>⚠️ Windows Limitation:</strong> The dropdown menu backgrounds cannot be customized and will always use Windows system colors. Only the menubar and individual menu item backgrounds (on hover/select) can be customized.</p>
    </div>
  </body>
</html>`,
					}).Show()
