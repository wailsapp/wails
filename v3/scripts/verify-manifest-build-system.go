...
func main() {
	...
	iosDeviceProfile := flag.String("ios-device-profile", "ios-device", "complete YAML profile used by -ios-device; it must select ios/arm64, destination=device, IPA and signing")
	sign := flag.Bool("sign", false, "run host signing with credentials already declared in wails.yaml")
