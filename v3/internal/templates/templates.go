...
func Install(options *flags.Init) error {
	...
	// If project directory already exists and is not empty, error (unless Force is set)
	if _, err := os.Stat(templateData.ProjectDir); !os.IsNotExist(err) {
		...
		if len(files) > 0 && !options.Force {
			return fmt.Errorf("project directory '%s' already exists and is not empty. Use -f to force init in a non-empty directory", templateData.ProjectDir)
		}
	}
