 ...
 * Uses HEAD request to check existence and content type, then injects a script tag.
 * Silently ignores if the script doesn't exist or if the server returns a non-JavaScript
 * response (e.g. an HTML fallback page from a SPA dev server like Vite).
 */
export function loadOptionalScript(url: string): Promise<void> {
    return fetch(url, { method: 'HEAD' })
        .then(response => {
            const contentType = response.headers.get('Content-Type') || '';
            const isJavaScript = contentType.startsWith('application/javascript') ||
