...
func TestWailsConfigEndpointWritesValidYAMLFromProjectState(t *testing.T) {
...
func TestWailsConfigEndpointPreservesBuildIntentWhenProjectStateChanges(t *testing.T) {
	...
	original := `# This user-owned comment must survive wizard edits.
version: 3
project:
...
func TestWailsConfigEndpointRejectsIncompleteProjectStateWithoutWritingYAML(t *testing.T) {
	t.Setenv("WAILS_EXP_USE_WAKE", "1")
	originalDirectory, err := os.Getwd()
	require.NoError(t, err)
	t.Cleanup(func() { require.NoError(t, os.Chdir(originalDirectory)) })
...
func TestWailsConfigEndpointDoesNotOverwriteLegacyWailsYAML(t *testing.T) {
	t.Setenv("WAILS_EXP_USE_WAKE", "1")
	originalDirectory, err := os.Getwd()
	require.NoError(t, err)
	t.Cleanup(func() { require.NoError(t, os.Chdir(originalDirectory)) })
	root := t.TempDir()
	require.NoError(t, os.Chdir(root))
	original := []byte("# user-owned legacy config\ninfo:\n  productName: Legacy Product\n")
	require.NoError(t, os.WriteFile(filepath.Join(root, manifest.Filename), original, 0o640))
	...
	mux.ServeHTTP(response, httptest.NewRequest(http.MethodPost, "/api/wails-config", strings.NewReader(`{
  "info": {
    "productName": "Native Product",
    "productIdentifier": "com.example.native",
    "version": "1.0.0"
  }
}`)))

	assert.Equal(t, http.StatusConflict, response.Code)
	assert.Contains(t, response.Body.String(), "legacy wails.yaml already exists")
