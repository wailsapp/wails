...
export default function InitFlow({ data, theme, toggleTheme }: { data: InitData; theme: Theme; toggleTheme: () => void; }) {
  ...
  static createFrom(src) {
    return new Person(src);
              ...
              {step === 'details' && (
                <Page key="details" title="Project details" subtitle={`Written to ${data.buildConfigFile}`} onBack={goBack} onNext={create} nextLabel={creating ? 'Creating…' : 'Create Project'} busy={creating} nextDisabled={!form.projectName.trim()}>
                  <div className="max-w-md mx-auto w-full space-y-3">
                    ...
                    <p className="text-xs text-gray-400 dark:text-gray-500">Advanced config (icons, file associations, custom URL schemes, iOS overrides) can be edited in <code className="font-mono">{data.buildConfigFile}</code> after creation.</p>
                    {error && (
