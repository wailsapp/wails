...
func TestYAMLProfilePlansTheCompleteTargetAndFormatMatrix(t *testing.T) {
	root := t.TempDir()
	contents := `version: 3
project:
  name: matrix
...
func TestYAMLTargetOverridesReachTheResolvedPipelineSpecs(t *testing.T) {
	root := t.TempDir()
	contents := `version: 3
project:
  name: custom
...
func TestYAMLProfileDestinationSelectsIOSDeviceOrSimulator(t *testing.T) {
	root := t.TempDir()
	contents := `version: 3
project:
  name: destinations
...
func TestYAMLProfileSigningWithoutFormatsSignsTheRunnable(t *testing.T) {
	root := t.TempDir()
	contents := `version: 3
project:
  name: signed-default
...
func TestYAMLProfileRejectsTargetsOutsideTheClosedV3Registry(t *testing.T) {
	root := t.TempDir()
	contents := `version: 3
project:
  name: destinations
...
func TestYAMLPlanTracksLocalGoModuleReplacements(t *testing.T) {
	parent := t.TempDir()
...
project:
  ...
  version: 1.0.0`), 0o644))
	...
	assert.True(t, found, "YAML-first plans must snapshot local replacement modules")
}

func TestYAMLPlanTracksGoWorkspaceModules(t *testing.T) {
	workspace := t.TempDir()
...
project:
  ...
  version: 1.0.0`), 0o644))
	...
	assert.True(t, found, "YAML-first plans must snapshot modules listed by a parent go.work")
}

func TestYAMLMultiTargetExecutorSharesProjectStages(t *testing.T) {
	root := t.TempDir()
