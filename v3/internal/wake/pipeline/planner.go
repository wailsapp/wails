...
func PlanBuild(config manifest.Config, request Request) (Plan, error) {
	...
	if err != nil {
		if config.Selected.Name != "" {
			field := `profiles[` + strconv.Quote(config.Selected.Name) + `]`
	...
	for _, outcome := range outcomes {
		...
		if err != nil {
			field := `targets[` + strconv.Quote(outcome.target.OS+"/"+outcome.target.Arch) + `]`
			if config.Selected.Name != "" {
				field = `profiles[` + strconv.Quote(config.Selected.Name) + `].targets[` + strconv.Quote(outcome.target.OS+"/"+outcome.target.Arch) + `]`
			}
			return Plan{}, manifest.AnnotateValidationError(err, config.Origins, field)
		}
...
func planTarget(config manifest.Config, request Request, multiTarget bool) (Plan, error) {
	...
	if !capability.SupportsToolchain(targetSettings.Toolchain) {
		...
		return Plan{}, manifest.AnnotateValidationError(err, config.Origins, `targets[`+strconv.Quote(target)+`].toolchain`)
	}
...
func originsForNode(config manifest.Config, node Node, target string) []OriginReference {
	...
	case GenerateBindings:
		prefixes = []string{"frontend.bindings", "build.tags", "build.obfuscated", `targets["` + target + `"].tags`, `targets["` + target + `"].obfuscated`}
	case BuildFrontend:
		prefixes = []string{"frontend.directory", "frontend.build", "frontend.output", "frontend.environment"}
	case CompileApplication:
		prefixes = append(prefixes, "build", platform, `targets["`+target+`"]`)
	case MergeUniversalBinaries:
		prefixes = append(prefixes, "build.output", `targets["`+target+`"]`)
	case GeneratePlatformAssets:
		prefixes = []string{"project", platform, "file_associations", "protocols"}
	case AssembleApplication, PackageArtifact:
		prefixes = []string{"project", "build.output", platform, `targets["` + target + `"]`, "file_associations", "protocols"}
		if spec, ok := node.Spec.(PackageSpec); ok {
			prefixes = append(prefixes, `packages["`+spec.Format+`"]`)
		}
	case SignArtifact:
	...
	case RunHook:
		if spec, ok := node.Spec.(HookSpec); ok {
			prefixes = []string{`hooks["` + string(spec.Phase) + `"]`}
		}
	}
	if profile != "" {
		prefixes = append(prefixes, `profiles["`+profile+`"].targets["`+target+`"]`)
	}
