...
func annotateHostConfigurationError(config manifest.Config, node Node, err error) error {
	...
	profileTarget := func() string {
		...
		if profile != "" && target != "" {
			return `profiles[` + strconv.Quote(profile) + `].targets[` + strconv.Quote(target) + `]`
		}
		if target != "" {
			return `targets[` + strconv.Quote(target) + `]`
		}
		return "build"
	}
	switch node.Kind {
	...
	case CompileApplication:
		...
		if errors.As(err, &fieldError) {
			...
			case "toolchain":
				field = `targets[` + strconv.Quote(target) + `].toolchain`
			case "obfuscated":
				targetField := `targets[` + strconv.Quote(target) + `].obfuscated`
