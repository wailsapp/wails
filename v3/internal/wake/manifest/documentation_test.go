...
func TestPublishedYAMLGuideExamplesMatchTheManifestSchema(t *testing.T) {
	guide := repositoryDocumentationPath("experimental", "yaml-builds.mdx")
	data, err := os.ReadFile(guide)
	require.NoError(t, err)
	examples := fencedExamples(strings.ReplaceAll(string(data), "\r\n", "\n"), "yaml")
	require.NotEmpty(t, examples)
	for index, example := range examples {
		if runtime.GOOS == "windows" && strings.Contains(example, "hooks:") {
		...
		_, err := decodeYAML(root, guide, []byte(example), "")
		require.NoErrorf(t, err, "YAML example %d", index+1)
		_, err = decodeYAML(root, guide, []byte(example), "release")
		require.NoErrorf(t, err, "YAML example %d release profile", index+1)
	}
}

func TestPublishedFieldReferenceIsCurrent(t *testing.T) {
	data, err := os.ReadFile(repositoryDocumentationPath("reference", "wails-yaml-fields.md"))
	require.NoError(t, err)
...
func TestPublishedJSONSchemaIsCurrent(t *testing.T) {
	want, err := JSONSchema()
	require.NoError(t, err)
	actual, err := os.ReadFile(repositoryDocumentationPublicPath("schemas", "wails.v3.json"))
	require.NoError(t, err)
...
func repositoryDocumentationPublicPath(parts ...string) string {
	path := filepath.Join("..", "..", "..", "..", "docs", "public")
	return filepath.Join(append([]string{path}, parts...)...)
}
