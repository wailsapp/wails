...
func collectSchemaFields(node *schemaNode, parent string, result *[]SchemaField) {
	...
	for _, name := range node.blockOrder {
		...
		if descriptor.repeated {
			...
			fieldType = "map(object)"
		}
		path := schemaPath(parent, pathName)
		*result = append(*result, SchemaField{
			Path: path, Type: fieldType, Required: descriptor.required, Example: "{}",
			Description: schemaBlockDescription(name), Platforms: descriptor.platforms, Formats: descriptor.formats,
		})
...
var schemaBlockDescriptions = map[string]string{
	"run":              "Application launch defaults.",
	"project":          "Project identity and metadata.",
	"frontend":         "Frontend commands, output and bindings settings.",
	"bindings":         "Generated frontend bindings settings.",
	"build":            "Native production build settings.",
	"dev":              "Development build and watcher settings.",
	"windows":          "Windows platform settings.",
	"darwin":           "macOS platform settings.",
	"linux":            "Linux platform settings.",
	"ios":              "iOS platform settings.",
	"android":          "Android platform settings.",
	"target":           "Named target overrides.",
	"package":          "Named package format settings.",
	"profile":          "Named production build profiles.",
	"file_association": "Named file association registrations.",
	"protocol":         "Named custom protocol registrations.",
	"hook":             "Named build lifecycle hooks.",
	"signing":          "Platform signing settings.",
	"notarization":     "macOS notarization settings.",
}

func schemaBlockDescription(name string) string {
	if description := schemaBlockDescriptions[name]; description != "" {
		return description
	}
...
func schemaDescription(path, name string) string {
	...
	if path == "version" {
		return "Wails manifest schema version; this must be the first field."
	}
	if strings.HasPrefix(path, `hooks[`) && name == "directory" {
