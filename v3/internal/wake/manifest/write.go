...
func EncodeDocument(doc Document) ([]byte, error) {
	// Source digests are accepted when loading manifests generated by early
	// migration previews, but they are workflow state owned by the hidden
	// migration report rather than user build intent. Never write them back
	// into wails.toml.
	if doc.Wake.Migration != nil {
		migration := *doc.Wake.Migration
		migration.Sources = nil
		doc.Wake.Migration = &migration
	}
