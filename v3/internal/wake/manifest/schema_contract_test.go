...
func TestPublishedPrototypeManifestsConform(t *testing.T) {
	prototypes, err := filepath.Glob(filepath.Join("testdata", "prototypes", "*.yaml"))
	require.NoError(t, err)
...
linux:
  desktop_entry: assets/application.desktop`)
	...
	encoded, err := EncodeConfig(loaded.Config)
	require.NoError(t, err)
	roundTripPath := filepath.Join(root, "round-trip.yaml")
...
targets:
  windows/amd64:
	...
	assert.Equal(t, `targets["windows/amd64"].toolchain`, validation.Field)
	assert.Equal(t, SourceRange{Filename: path, StartLine: 9, StartColumn: 16, EndLine: 9, EndColumn: 21}, validation.Range)
}
...
targets:
  ...
  windows/amd64: {}`)
	...
	var validation *ValidationError
	require.True(t, errors.As(err, &validation))
	assert.Equal(t, `targets.windows/amd64`, validation.Field)
	assert.Equal(t, SourceRange{Filename: path, StartLine: 9, StartColumn: 3, EndLine: 9, EndColumn: 16}, validation.Range)
}
...
project:
  ...
  version: 1.0.0`)
	...
	assert.Equal(t, SourceRange{Filename: path, StartLine: 3, StartColumn: 3, EndLine: 3, EndColumn: 4}, validation.Range)
}
...
targets:
  windows/amd64:
	...
	assert.Equal(t, SourceRange{Filename: path, StartLine: 8, StartColumn: 11, EndLine: 8, EndColumn: 18}, loaded.Config.Origins["build.output"].Range)
	assert.Equal(t, OriginManifest, loaded.Config.Origins[`targets["windows/amd64"].toolchain`].Kind)
	assert.Equal(t, 11, loaded.Config.Origins[`targets["windows/amd64"].toolchain`].Range.StartLine)
...
func TestSchemaReferenceDerivesFieldsTypesAndDefaults(t *testing.T) {
	...
	assert.Equal(t, "string", byPath[`targets["target"].toolchain`].Type)
	assert.Equal(t, "list(string)", byPath[`profiles["profile"].targets["target"].formats`].Type)
	assert.Equal(t, "string", byPath[`packages["format"].install_scope`].Type)
	assert.True(t, byPath["version"].Required)
	assert.True(t, byPath["project"].Required)
	assert.True(t, byPath[`profiles["profile"].targets`].Required)
	assert.True(t, byPath[`file_associations["association"].extensions`].Required)
	assert.Contains(t, string(SchemaReferenceMarkdown()), "| `build.output` | string | no | `bin` | `\"bin\"` |")
	assert.Equal(t, "nsis", byPath[`packages["format"].install_scope`].Formats)
}
...
targets:
  linux/amd64:
	...
	encoded, err := EncodeConfig(loaded.Config)
	require.NoError(t, err)
	assert.Contains(t, string(encoded), "linux/amd64:\n    obfuscated: false")
...
func TestEjectionCoversEveryPlatformSchemaField(t *testing.T) {
	...
	for _, platform := range []string{"windows", "darwin", "linux", "ios", "android"} {
		...
		assert.Equal(t, expectedAttributes, sortedEjectedAttributeNames(block), platform)

		signingBlock := requireEjectedBlock(t, block, "signing", "")
		...
		assert.Equal(t, expectedSigning, sortedEjectedAttributeNames(signingBlock), platform+" signing")
		if platform == "darwin" {
			notarization := requireEjectedBlock(t, block, "notarization", "")
			assert.Equal(t, []string{"credential"}, sortedEjectedAttributeNames(notarization))
		} else {
			assert.Nil(t, findEjectedBlock(block, "notarization", ""), platform)
		}
	}
}
...
func TestEjectionCoversEveryPackageSchemaField(t *testing.T) {
	...
	for _, format := range []string{"nsis", "msix", "dmg", "appimage", "deb", "rpm", "archlinux"} {
		t.Run(format, func(t *testing.T) {
			...
			assert.Equal(t, expected, sortedEjectedAttributeNames(block))
		})
	}
	...
	for _, format := range []string{"nsis", "dmg", "deb", "rpm", "archlinux"} {
		t.Run(format+" template", func(t *testing.T) {
			...
			require.NoError(t, err)
			block := requireEjectedBlock(t, parseEjectedBody(t, encoded), "package", format)
			assert.Equal(t, []string{"template"}, sortedEjectedAttributeNames(block))
		})
	}
}
...
profiles:
  release:
	...
	var validation *ValidationError
	require.ErrorAs(t, err, &validation)
	assert.Equal(t, `profiles["release"].targets["plan9/amd64"]`, validation.Field)
	assert.Equal(t, path, validation.Range.Filename)
	assert.Equal(t, 10, validation.Range.StartLine)
}

func parseEjectedBody(t *testing.T, source []byte) *yaml.Node {
	t.Helper()
...
func requireEjectedBlock(t *testing.T, body *yaml.Node, blockType, label string) *yaml.Node {
	t.Helper()
...
func findEjectedBlock(body *yaml.Node, blockType, label string) *yaml.Node {
	if label == "" {
		return mappingValue(body, blockType)
	}
	group := mappingValue(body, yamlRepeatedName(blockType))
	if group == nil {
		return nil
	}
	return mappingValue(group, label)
}

func sortedEjectedAttributeNames(body *yaml.Node) []string {
	result := make([]string, 0, len(body.Content)/2)
	for index := 0; index+1 < len(body.Content); index += 2 {
		name := body.Content[index].Value
		if name != "signing" && name != "notarization" {
