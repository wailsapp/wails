...
var diagnosticYAMLAssignment = regexp.MustCompile(`^(\s*[^#][^:]*:\s*).*$`)
...
func validationSuggestion(validation *ValidationError) string {
	if strings.Contains(validation.Detail, "unsupported field") {
	...
	if strings.Contains(validation.Field, ".signing") || strings.Contains(validation.Field, ".notarization") {
		return "configure the named signing field in this platform block; keep secret values outside wails.yaml"
	}
	return ""
}
...
func redactDiagnosticSourceLine(line, field string) string {
	...
	return diagnosticYAMLAssignment.ReplaceAllString(line, `${1}<redacted>`)
}
