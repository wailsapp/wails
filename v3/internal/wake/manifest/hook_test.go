...
func TestHookYAMLRoundTripAndValidation(t *testing.T) {
	root := t.TempDir()
...
hooks:
  before_build:
    ...
    cache: true
    inputs: [version.txt]
    outputs: [generated/version.go]`, script)
	...
	require.NoError(t, err)
	assert.Contains(t, string(encoded), "hooks:\n  before_build:")
	second := t.TempDir()
...
func TestHookValidationCarriesTheYAMLSourceRange(t *testing.T) {
...
hooks:
  before_build:
	...
	assert.Equal(t, `hooks["before_build"].script`, validation.Field)
