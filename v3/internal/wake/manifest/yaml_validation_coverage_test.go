...
func validYAMLValidationConfig(root string) Config {
...
func TestYAMLValidationRejectsInvalidCoreConfiguration(t *testing.T) {
	root := t.TempDir()
	tests := []struct {
		name   string
		mutate func(*Config)
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			config := validYAMLValidationConfig(root)
			test.mutate(&config)
...
func TestYAMLValidationRejectsAnUnresolvableProjectRoot(t *testing.T) {
	config := validYAMLValidationConfig(filepath.Join(t.TempDir(), "missing"))
	require.Error(t, validateConfig(config))
}
...
profiles:
  release:
    targets:
      linux/amd64:
        formats: [deb, deb]`)
	_, err := decodeYAML(root, filepath.Join(root, Filename), source, "")
...
func TestYAMLValidationCoversPackageTemplatesAndOptions(t *testing.T) {
	root := t.TempDir()
...
func TestYAMLValidationRejectsEveryConfigurablePathIntoWails(t *testing.T) {
	root := t.TempDir()
	tests := []struct {
		field  string
		mutate func(*Config)
		...
		{`file_associations[""].icon`, func(c *Config) {
			c.Associations = []Association{{Extensions: []string{"txt"}, Icon: ".wails/file.ico"}}
		}},
		{`packages["msix"].manifest`, func(c *Config) { c.Package.Windows.MSIX.Manifest = ".wails/AppxManifest.xml" }},
		{`packages["dmg"].background`, func(c *Config) { c.Package.Darwin.DMG.Background = ".wails/background.png" }},
		{`packages["dmg"].files`, func(c *Config) { c.Package.Darwin.DMG.Files = map[string]string{"Read Me": ".wails/README.md"} }},
	}
	for _, test := range tests {
...
func TestYAMLCorePathHelpersCoverResolvedPaths(t *testing.T) {
	root := t.TempDir()
...
func TestYAMLDefaultsAndBinaryNameDerivationCoverAllArchitectures(t *testing.T) {
...
func TestYAMLWritersAndDiscoveryReportInvalidRequests(t *testing.T) {
	root := t.TempDir()
	project := Project{Name: "app", ProductName: "App", Identifier: "com.example.app", Version: "1.0.0"}
