  ...
  linux:
    ...
    run:
      ...
      environment:
        OVERRIDE: platform
        PLATFORM: yes`
	loaded, err := decodeYAML(root, filepath.Join(root, Filename), []byte(source), "")
	...
	check(loaded.Config)
	ejected, err := EncodeEjectedYAML(loaded.Config, "test")
	require.NoError(t, err)
	restored, err := decodeYAML(root, filepath.Join(root, EjectedFilename), ejected, "")
	require.NoError(t, err)
...
targets:
  ...
  android:
	...
	require.NoError(t, err)
	restored, err := decodeYAML(root, filepath.Join(root, EjectedFilename), encoded, "")
	require.NoError(t, err)
	check(restored.Config)
}
