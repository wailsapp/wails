...
func Discover(start string) (root, path string, err error) {
	return discoverNativeWithOperations(start, filepath.Abs, os.Stat, os.ReadFile)
}
...
func discoverWithOperations(start string, absolute func(string) (string, error), stat func(string) (fs.FileInfo, error)) (root, path string, err error) {
	return discoverNativeWithOperations(start, absolute, stat, func(string) ([]byte, error) {
		return []byte("version: 3\nproject: {}\n"), nil
	})
}

func discoverNativeWithOperations(start string, absolute func(string) (string, error), stat func(string) (fs.FileInfo, error), readFile func(string) ([]byte, error)) (root, path string, err error) {
	...
	for {
		...
		if info, statErr := stat(candidate); statErr == nil {
			...
			contents, readErr := readFile(candidate)
			if readErr != nil {
				return "", "", readErr
			}
			if !legacyWailsYAML(contents) {
...
// legacyWailsYAML identifies the old setup metadata file by its unambiguous
// top-level info section. Everything else is treated as a native candidate so
// malformed native manifests fail loudly instead of silently running Taskfiles.
func legacyWailsYAML(contents []byte) bool {
	var document yaml.Node
	if err := yaml.Unmarshal(contents, &document); err != nil || len(document.Content) != 1 || document.Content[0].Kind != yaml.MappingNode {
		return false
	}
	root := document.Content[0]
	return mappingValue(root, "info") != nil && mappingValue(root, "project") == nil
}
...
func loadWithOperations(start, profile string, discover func(string) (string, string, error), readFile func(string) ([]byte, error)) (*Loaded, error) {
	...
	return decodeYAML(root, path, raw, profile)
}
...
func defaults(project Project) Document {
	...
	frontendRaw := &manifestFrontend{}
	applySchemaDefaults(frontendRaw)
	bindingsRaw := &manifestBindings{}
...
func validateConfig(config Config) error {
	...
	for _, platformEntry := range []struct {
		...
		for _, targetEntry := range []struct {
			...
			if target.Toolchain != "" && !contains([]string{"auto", "native", "zig", "docker"}, target.Toolchain) {
				return fieldValidationError(fmt.Sprintf(`targets[%q].toolchain`, name+"/"+arch), "unsupported toolchain %q", target.Toolchain)
			}
			if err := validateApplicationArgs(fmt.Sprintf(`targets[%q].dev.args`, name+"/"+arch), target.Dev.Args); err != nil {
				return err
			}
			if err := validateEnvironment(fmt.Sprintf(`targets[%q].run.environment`, name+"/"+arch), target.Run.Environment); err != nil {
				return err
			}
			if err := validateEnvironment(fmt.Sprintf(`targets[%q].environment`, name+"/"+arch), target.Environment); err != nil {
				return err
			}
		}
	...
	for _, association := range config.Associations {
		field := fmt.Sprintf(`file_associations[%q]`, association.Name)
	...
	for _, protocol := range config.Protocols {
		field := fmt.Sprintf(`protocols[%q]`, protocol.Scheme)
...
func validateHooks(root string, paths *projectPathValidator, hooks map[HookPhase]Hook) error {
	...
	for _, phaseName := range phases {
		...
		hook := hooks[phase]
		field := fmt.Sprintf(`hooks[%q]`, phase)
...
func packageManifestField(name string) string {
	...
	return fmt.Sprintf(`packages[%q]`, format)
}
