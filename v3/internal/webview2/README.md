...
that produces them. Consumed by the Wails v3 runtime on Windows.

## Versioning

This package is part of the `github.com/wailsapp/wails/v3` module and lives
under `internal/`, so it versions and ships with v3 itself — there are no
separate webview2 tags or releases. Binding changes reach users through
normal v3 releases; SDK regenerations land as ordinary PRs from the update
workflow.

Historical `webview2/v1.x` tags belong to the retired standalone
`github.com/wailsapp/wails/webview2` module. They remain resolvable for
existing consumers but are frozen; all new work happens here.
...
```sh
cd v3/internal/webview2/scripts
...
# Record an SDK bump in CHANGELOG.md (added/removed interfaces,
...
Run `go generate ./...` from `v3/internal/webview2/` to invoke `full` via the
...
```go
import "github.com/wailsapp/wails/v3/internal/webview2/pkg/webview2"
