...
Each box is one Go package under `v3/internal/webview2/scripts/`; `webview2gen` (the
...
The bindings are part of the `github.com/wailsapp/wails/v3` module (this
directory sits under `v3/internal/`, so nothing outside v3 can import
them). Only `scripts/go.mod` remains a separate dev-time module
(`module updater`, a legacy name kept to avoid a noisy rename). The split:

- Lets `scripts/` depend on developer-only packages (NuGet fetcher, HTTP
  client, participle/v2) without adding those deps to the v3 module.
- Lets `scripts/` use a separate, sometimes-older Go version (1.20) than
  v3 without conflict.

Run go commands in `scripts/` with `GOWORK=off` — it is intentionally not
part of the repo workspace.
