...
func (p *Param) SetupCode(w io.Writer) error {
	if p.setupTemplate == "" {
		return nil
	}
	data := struct {
	...
	return renderTemplate("Param Setup: "+p.setupTemplate, p.setupTemplate, &data, w)
}
func (p *Param) CleanupCode(w io.Writer) error {
	if p.cleanupTemplate == "" {
		return nil
	}
	return renderTemplate("Param Cleanup: "+p.cleanupTemplate, p.cleanupTemplate, p, w)
}
