...
run:
  ...
  environment:
    RUN_VALUE: configured`
	require.NoError(t, os.WriteFile(filepath.Join(root, "wails.yaml"), []byte(source), 0600))
	...
	require.NoError(t, err)
	require.NoError(t, os.WriteFile(filepath.Join(root, "wails.yaml"), []byte(strings.ReplaceAll(source, "configured", "changed")), 0600))
...
`), 0600))
	...
	require.NoError(t, os.WriteFile("wails.yaml", []byte("invalid"), 0600))
	t.Setenv("WAILS_EXP_USE_WAKE", "1")
