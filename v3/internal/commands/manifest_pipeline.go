...
func (h *manifestHandler) stageAssociationIcons(generatedRoot, targetOS string, associations []manifest.Association) ([]manifest.Association, error) {
	...
	for index := range result {
		...
		if err != nil {
			return nil, fmt.Errorf("file_associations[%q].icon: %w", result[index].Name, err)
		}
		...
		case "android":
			...
			result[index].Icon = name
		default:
			return nil, fmt.Errorf("file_associations[%q].icon: unsupported target %s", result[index].Name, targetOS)
		}
		if err != nil {
			return nil, fmt.Errorf("file_associations[%q].icon: %w", result[index].Name, err)
		}
...
func (h *manifestHandler) sign(ctx context.Context, s pipeline.SignSpec) (pipeline.RunResult, error) {
	...
	password := ""
	if s.TargetOS == "windows" && s.Config.Credential != "" {
		password = os.Getenv(s.Config.Credential)
		if password == "" {
			return pipeline.RunResult{}, fmt.Errorf("windows signing password environment variable %s is empty", s.Config.Credential)
		}
	}
