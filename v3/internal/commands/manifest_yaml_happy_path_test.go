...
const yamlBuildFixture = `version: 3
project:
  name: hello
  product_name: Hello
  company: Example
...
func TestBuildPlanUsesYAMLProfileAndDoesNotInvokeTaskfile(t *testing.T) {
	t.Setenv("WAILS_EXP_USE_WAKE", "1")
	root := t.TempDir()
	prepareYAMLPlanInputs(t, root)
	yaml := yamlBuildFixture + `
profiles:
  release:
    targets:
      linux/amd64:
      ...
      windows/amd64:
        formats: [nsis]
`
	require.NoError(t, os.WriteFile(filepath.Join(root, manifest.Filename), []byte(yaml), 0o644))
	...
	output := captureYAMLStdout(t, func() {
		err := Build(&flags.Build{Plan: true}, []string{"release"})
		require.NoError(t, err)
	})
	assert.False(t, called, "wails.yaml must select the native route even when a Taskfile exists")
...
func TestBuildPlanAcceptsCommaSeparatedTargets(t *testing.T) {
	...
	root := t.TempDir()
	prepareYAMLPlanInputs(t, root)
	require.NoError(t, os.WriteFile(filepath.Join(root, manifest.Filename), []byte(yamlBuildFixture), 0o644))
...
func TestBuildPlanShowsResolvedAnonymousCompilerOverrides(t *testing.T) {
	...
	output := captureYAMLStdout(t, func() { require.NoError(t, Build(&options, nil)) })
	toolchain := "native"
	...
	jsonOutput := captureYAMLStdout(t, func() { require.NoError(t, Build(&options, nil)) })
...
func prepareYAMLPlanInputs(t *testing.T, root string) {
	t.Helper()
...
func TestDevPlanUsesTheYAMLBuildPipeline(t *testing.T) {
	t.Setenv("WAILS_EXP_USE_WAKE", "1")
	prependFakePlanTools(t, "npm")
	root := t.TempDir()
	prepareYAMLPlanInputs(t, root)
...
func TestYAMLEjectWritesAnInactiveResolvedManifestAndRequiresForce(t *testing.T) {
	root := t.TempDir()
	t.Chdir(root)
	yaml := `version: 3
project:
...
func TestYAMLSignCommandRunsTheCompleteProfilePipeline(t *testing.T) {
	t.Setenv("WAILS_EXP_USE_WAKE", "1")
	if runtime.GOOS != "linux" {
...
func TestYAMLPackageCommandBuildsTheRequestedTargetAndFormat(t *testing.T) {
	t.Setenv("WAILS_EXP_USE_WAKE", "1")
	if runtime.GOOS != "linux" {
...
func TestYAMLLinuxPackageTemplateReachesTheOwnedPackageWorkspace(t *testing.T) {
	root := t.TempDir()
	t.Chdir(root)
...
func TestYAMLBuildExecutesFrontendAndRealGoCompile(t *testing.T) {
	root := t.TempDir()
	t.Chdir(root)
	require.NoError(t, os.WriteFile(filepath.Join(root, manifest.Filename), []byte(yamlBuildFixture), 0o644))
	...
	assert.False(t, taskCalled, "the YAML-first pipeline must not invoke Taskfile execution")
	...
	require.NoError(t, err)
	assert.Equal(t, "install\nrun bundle\n", readTestFile(t, invocationLog), "a warm YAML build should use the cache")
	...
	require.NoError(t, err)
	assert.Equal(t, "install\nrun bundle\nrun bundle\ninstall\nrun bundle\n", readTestFile(t, invocationLog), "--force must rebuild cached YAML stages")
}

func TestYAMLBuildExecutesLifecycleHooksAroundARealCompile(t *testing.T) {
	root := t.TempDir()
	t.Chdir(root)
	yaml := yamlBuildFixture + `
hooks:
...
func TestYAMLDevBuildExecutesTheDevelopmentPipeline(t *testing.T) {
	root := t.TempDir()
	t.Chdir(root)
	require.NoError(t, os.WriteFile(filepath.Join(root, manifest.Filename), []byte(yamlBuildFixture), 0o644))
...
func TestYAMLPackagePlanGeneratesOwnedPlatformAssets(t *testing.T) {
	root := t.TempDir()
	t.Chdir(root)
	yaml := `version: 3
project:
...
func TestYAMLDarwinAppPackageUsesGeneratedPlatformAssets(t *testing.T) {
	root := t.TempDir()
	t.Chdir(root)
	require.NoError(t, os.MkdirAll(filepath.Join(root, "packaging"), 0o755))
...
func TestYAMLDMGPackagingBuildsTheAppAndResolvesOptions(t *testing.T) {
	root := t.TempDir()
	t.Chdir(root)
	require.NoError(t, os.MkdirAll(filepath.Join(root, "packaging"), 0o755))
...
func TestYAMLGeneratesAssetsForEverySupportedPlatform(t *testing.T) {
	root := t.TempDir()
	t.Chdir(root)
	yaml := `version: 3
project:
...
func TestYAMLTargetBuildNumbersAndMinimumVersionsReachGeneratedAssets(t *testing.T) {
	root := t.TempDir()
	t.Chdir(root)
	yaml := `version: 3
project:
...
func TestYAMLLinuxSigningProducesTheSignedArtifact(t *testing.T) {
	root := t.TempDir()
	t.Chdir(root)
	yaml := `version: 3
project:
...
func TestYAMLAppImagePackagingUsesTheConfiguredOutput(t *testing.T) {
	root := t.TempDir()
	t.Chdir(root)
	require.NoError(t, os.MkdirAll(filepath.Join(root, "packaging"), 0o755))
...
func TestYAMLDefaultAppImagePackagingGeneratesDesktopMetadata(t *testing.T) {
	root := t.TempDir()
	t.Chdir(root)
	yaml := `version: 3
project:
...
func TestYAMLPlanIdentityCoversEveryPortableStage(t *testing.T) {
	root := t.TempDir()
	t.Chdir(root)
	yaml := `version: 3
project:
...
func TestYAMLIOSSignedAppAndIPAUseDeclaredTargetSettings(t *testing.T) {
	root := t.TempDir()
	t.Chdir(root)
	require.NoError(t, os.MkdirAll(filepath.Join(root, "packaging"), 0o755))
...
func TestYAMLIOSSignedDefaultSimulatorAppUsesGeneratedInfoPlist(t *testing.T) {
	root := t.TempDir()
	t.Chdir(root)
	yaml := `version: 3
project:
...
func TestYAMLDarwinUniversalCompileUsesBothArchitectures(t *testing.T) {
	root := t.TempDir()
	t.Chdir(root)
	yaml := yamlBuildFixture + `
profiles:
...
func TestYAMLCompileUsesConfiguredGoFlagsAndTargetMinimum(t *testing.T) {
	tests := []struct {
		name, target, yaml, wantEnv string
	}{
		{
			name:   "linux flags",
			target: "linux/amd64",
  ...
  release:
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			root := t.TempDir()
			t.Chdir(root)
			require.NoError(t, os.WriteFile(filepath.Join(root, manifest.Filename), []byte(test.yaml), 0o644))
			loaded, err := manifest.Load(root, "release")
			require.NoError(t, err)
			plan, err := pipeline.PlanBuild(loaded.Config, pipeline.Request{Verb: "build"})
			require.NoError(t, err)
...
func TestYAMLAndroidCompileDiscoversNDKForEachHostToolchain(t *testing.T) {
	root := t.TempDir()
	t.Chdir(root)
	yaml := `version: 3
project:
...
func TestYAMLIOSSImulatorCompileUsesTheDefaultVariantAndMinimum(t *testing.T) {
	root := t.TempDir()
	t.Chdir(root)
	yaml := `version: 3
project:
...
func TestYAMLProjectIconIsCopiedIntoGeneratedPlatformAssets(t *testing.T) {
	root := t.TempDir()
	t.Chdir(root)
	_, sourceFile, _, ok := runtime.Caller(0)
	require.True(t, ok)
...
func TestYAMLFileAssociationIconsAreStagedAndPackagedWithoutMutatingTheSource(t *testing.T) {
	root := t.TempDir()
	t.Chdir(root)
	_, sourceFile, _, ok := runtime.Caller(0)
	require.True(t, ok)
...
func TestYAMLAndroidCompileUsesConfiguredNDKForBothArchitectures(t *testing.T) {
	previousHost := manifestHostOS
...
func TestYAMLWindowsCompileGeneratesTheResourceOverlay(t *testing.T) {
	root := t.TempDir()
...
func TestYAMLObfuscatedBuildUsesGarbleAndConfiguredArguments(t *testing.T) {
	root := t.TempDir()
	t.Chdir(root)
	yaml := `version: 3
project:
...
func TestYAMLLinuxCompileRejectsArchitecturesOutsideTheClosedV3Registry(t *testing.T) {
	root := t.TempDir()
	t.Chdir(root)
	yaml := yamlBuildFixture + `
profiles:
...
func TestYAMLMSIXPackagingUsesMakeAppxOnTheWindowsTarget(t *testing.T) {
	root := t.TempDir()
	t.Chdir(root)
	require.NoError(t, os.MkdirAll(filepath.Join(root, "packaging"), 0o755))
...
func TestYAMLNSISPackagingUsesTheARM64InstallerDefinition(t *testing.T) {
	root := t.TempDir()
	t.Chdir(root)
	require.NoError(t, os.MkdirAll(filepath.Join(root, "packaging"), 0o755))
...
func TestYAMLAndroidSigningProducesTheProductionAABArtifact(t *testing.T) {
	root := t.TempDir()
	t.Chdir(root)
	yaml := yamlBuildFixture + `
android:
...
func TestYAMLAndroidAMD64PackagingUsesTheX86ABI(t *testing.T) {
	root := t.TempDir()
	t.Chdir(root)
	yaml := yamlBuildFixture + `
profiles:
...
func TestYAMLAndroidPackagingUsesAUserOwnedManifest(t *testing.T) {
	root := t.TempDir()
	t.Chdir(root)
...
func captureYAMLStdout(t *testing.T, fn func()) string {
	t.Helper()
	old := os.Stdout
