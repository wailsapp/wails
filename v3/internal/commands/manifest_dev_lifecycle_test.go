...
dev:
  ...
  grace_period_ms: 100`
	...
	updatedYAML := strings.Replace(yaml, `dev: [npm, run, dev]`, `dev: [npm, run, serve]`, 1)
	require.NoError(t, os.WriteFile(filepath.Join(root, manifest.Filename), []byte(updatedYAML), 0o644))
	...
	failedFrontendYAML := strings.Replace(updatedYAML, `dev: [npm, run, serve]`, `dev: [npm, run, missing]`, 1)
	require.NoError(t, os.WriteFile(filepath.Join(root, manifest.Filename), []byte(failedFrontendYAML), 0o644))
	...
	watchYAML := strings.Replace(updatedYAML, `watch: ['**/*.go', wails.yaml]`, `watch: ['**/*.go', wails.yaml, README.md]`, 1)
	watchYAML = strings.Replace(watchYAML, `dev: [npm, run, serve]`, `dev: [npm, run, dev]`, 1)
	require.NoError(t, os.WriteFile(filepath.Join(root, manifest.Filename), []byte(watchYAML), 0o644))
	...
	watchServeYAML := strings.Replace(watchYAML, `dev: [npm, run, dev]`, `dev: [npm, run, serve]`, 1)
	require.NoError(t, os.WriteFile(filepath.Join(root, manifest.Filename), []byte(watchServeYAML), 0o644))
	...
	require.NoError(t, err)
	argumentYAML := strings.Replace(watchServeYAML, "profile with spaces.yaml", "another profile.yaml", 1)
	require.NoError(t, os.WriteFile(filepath.Join(root, manifest.Filename), []byte(argumentYAML), 0644))
...
func TestManifestDevRetainsWatcherWhenRefreshFails(t *testing.T) {
	...
	source := readTestFile(t, manifestPath)
	yaml := strings.Replace(source, "use_git_ignore: false", "use_git_ignore: true", 1)
	require.NotEqual(t, source, yaml)
