...
func RunApplication(options *RunOptions, args []string) (resultErr error) {
	...
	if errors.Is(err, fs.ErrNotExist) {
		if options.Device != "" || options.Emulator != "" || options.Destination != "" {
			return fmt.Errorf("mobile launch options require wails.yaml")
		}
	...
	if !features.WakeEnabled() {
		return fmt.Errorf("wails.yaml found: enable the experimental build system with WAILS_EXP_USE_WAKE=1 to run this project")
	}
