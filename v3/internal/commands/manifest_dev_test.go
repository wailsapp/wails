...
func TestLegacyDevRejectsManifestOnlyFlags(t *testing.T) {
	...
	assert.ErrorContains(t, err, "require an active wails.yaml")
}
...
func TestManifestDevValidatesBeforeStartingProcesses(t *testing.T) {
	...
	t.Run("malformed manifest", func(t *testing.T) {
		...
		assert.ErrorContains(t, err, "parse wails.yaml")
	})
...
func TestMatchesDevWatchDoubleStar(t *testing.T) {
	patterns := []string{"internal/**/generated/*.go", "**/*.go", "wails.yaml"}
	...
	assert.True(t, matchesDevWatch(patterns, "wails.yaml"))
...
func TestDevWatchSessionChanged(t *testing.T) {
	...
	next := base
	next.Dev.Watch = []string{"**/*.go", "wails.yaml"}
...
func BenchmarkMatchDevPathPattern(b *testing.B) {
	cases := []struct {
		...
		{pattern: "wails.yaml", path: "wails.yaml"},
...
func BenchmarkCompiledDevWatchMatcher(b *testing.B) {
	patterns := []string{"**/*.go", "internal/**/test?.go", "wails.yaml", "frontend/**/generated/**/*.ts"}
