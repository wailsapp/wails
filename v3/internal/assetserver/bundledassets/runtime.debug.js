...
async function postBatch(connID, frames) {
  var _a3;
  if (frames.length === 1 || frames.some((f) => f.byteLength > CHUNK_THRESHOLD2)) {
...
function validFraming(buf) {
  if (buf.byteLength < 9) return false;
  const dv = new DataView(buf);
  return dv.getUint8(0) === 87 && dv.getUint8(1) === 83 && dv.getUint8(2) === 49 && dv.getUint8(3) === 0;
}
