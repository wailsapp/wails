...
fetch("/wails/init.js").then((r) => r.ok && r.status !== 204 ? r.text() : null).then((js) => {
  if (js) eval(js);
}).catch(() => {
});
fetch("/wails/init.css").then((r) => r.ok && r.status !== 204 ? r.text() : null).then((css) => {
  if (css) {
