...
type finalizerContextRequest struct {
	Request
	ctx    context.Context
	closed atomic.Int32
}

func (r *finalizerContextRequest) Context() context.Context { return r.ctx }
func (r *finalizerContextRequest) Close() error             { r.closed.Add(1); return nil }

func TestFinalizerPreservesRequestCancellation(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	native := &finalizerContextRequest{ctx: ctx}
