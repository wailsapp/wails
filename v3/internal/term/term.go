...
// UpdateInfo updates the secondary info line shown below the spinner.
func (s Spinner) UpdateInfo(text string) {
	if s.state == nil || s.state.app == nil || s.state.infoPtr == nil {
		return
	}
	s.state.mu.Lock()
	*s.state.infoPtr = text
	s.state.mu.Unlock()
	s.state.app.RequestRender()
}
