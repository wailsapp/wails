...
with log.open('w') as f:
    ...
    try:
        ...
        r['binding_roundtrip'] = wait('YAML_BINDING_ROUNDTRIP', timeout=15) if r['runtime_ready'] else False
        r['stderr_logging'] = wait('YAML_STDERR_LOG_PASS', timeout=15) if r['runtime_ready'] else False
        if options.expected_app_args is not None:
            marker = 'YAML_APP_ARGS ' + json.dumps(options.expected_app_args, separators=(',', ':'), ensure_ascii=False)
            r['initial_application_args'] = wait(marker, timeout=15) if r['runtime_ready'] else False
        if r['runtime_ready']:
            frontend.write_text(original_frontend + "\nCall.ByName('main.AcceptanceService.Confirm', 'YAML_HMR_PASS');\n")
            r['frontend_hmr'] = wait('YAML_BINDING_ROUNDTRIP YAML_HMR_PASS', timeout=30)
            r['hmr_without_backend_restart'] = 'Backend rebuilt and restarted' not in log.read_text()
            baseline = log.read_text().count('YAML_BINDING_ROUNDTRIP')
            ...
            r['second_binding_roundtrip'] = wait('YAML_BINDING_ROUNDTRIP', baseline + 1, 15)
            if options.expected_app_args is not None:
