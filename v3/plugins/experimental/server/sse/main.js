...
function handleCallback(e) {
  ...
  const payload = JSON.parse(e.data);
    _wails.callResultHandler(payload.id,
							 payload.result,
							 true);
}
...
function handleCallbackError(e) {
    const payload = JSON.parse(e.data);
    _wails.callErrorHandler(payload.id, payload.result);
}

function handleDialog(e) {
    const payload = JSON.parse(e.data);
    _wails.dialogResultCallback(payload.id,
                          payload.result,
                          true);
}
...
function handleWailsEvent(e) {
	const payload = JSON.parse(e.data);
	_wails.dispatchWailsEvent(payload);
}
...
function _connect() {
    if (source == null && window._wails) {
        source = new EventSource("/server/events?clientId="+window._wails.clientId);
